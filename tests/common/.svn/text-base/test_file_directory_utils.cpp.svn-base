...
static const std::string basepath = "/tmp/__tmp__dir__";
...
TEST_F(TestFileDirectoryUtils, create_full_path)
{
  ...
  EXPECT_EQ(FileDirectoryUtils::create_full_path(invalidpath.c_str()), true);
...
TEST_F(TestFileDirectoryUtils, delete_directory)
{
  ...
  FSU::delete_directory_recursively(path.c_str());
  EXPECT_EQ(FileDirectoryUtils::create_full_path(path.c_str()), true);
...
TEST_F(TestFileDirectoryUtils, gen_tmpfile_name)
{
  const char *tmpdir = "/tmp";
  const char *prefix = "cs_sort_tmp_file";
  char file_name1[OB_MAX_FILE_NAME_LENGTH];
  char file_name2[OB_MAX_FILE_NAME_LENGTH];

  EXPECT_FALSE(FSU::gen_tmpfile_name(NULL, OB_MAX_FILE_NAME_LENGTH, NULL, NULL));
  EXPECT_FALSE(FSU::gen_tmpfile_name(file_name1, OB_MAX_FILE_NAME_LENGTH, NULL, NULL));
  EXPECT_FALSE(FSU::gen_tmpfile_name(file_name1, 0, tmpdir, NULL));

  EXPECT_TRUE(FSU::gen_tmpfile_name(file_name1, OB_MAX_FILE_NAME_LENGTH, tmpdir, NULL));
  EXPECT_FALSE(FSU::exists(file_name1));

  EXPECT_TRUE(FSU::gen_tmpfile_name(file_name1, OB_MAX_FILE_NAME_LENGTH, tmpdir, prefix));
  std::string path = std::string(tmpdir) + "/" + prefix;
  EXPECT_EQ(0, strncmp(file_name1, path.c_str(), path.length()));

  EXPECT_TRUE(FSU::gen_tmpfile_name(file_name1, OB_MAX_FILE_NAME_LENGTH, tmpdir, prefix));
  EXPECT_TRUE(FSU::gen_tmpfile_name(file_name2, OB_MAX_FILE_NAME_LENGTH, tmpdir, prefix));
  EXPECT_FALSE(FSU::exists(file_name1));
  EXPECT_FALSE(FSU::exists(file_name2));
  EXPECT_STRNE(file_name1, file_name2);

  // buffer not enough test
  int64_t len = strlen(file_name1);
  EXPECT_FALSE(FSU::gen_tmpfile_name(file_name1, len, tmpdir, prefix));
  EXPECT_TRUE(FSU::gen_tmpfile_name(file_name1, len + 1, tmpdir, prefix));
}
...
TEST_F(TestFileDirectoryUtils, delete_direcotry_recursively)
{
  ...
  files.close();
  EXPECT_EQ(FileDirectoryUtils::delete_directory_recursively(basepath.c_str()), true);
