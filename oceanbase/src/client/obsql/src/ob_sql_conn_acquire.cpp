...
int release_conn(ObSQLConn *conn)
{
  ...
  pthread_mutex_lock(&pool_mutex);
  ob_sql_list_del(&conn->conn_list_node_);
  ob_sql_list_add_tail(&conn->conn_list_node_, &conn->pool_->conn_list_.free_conn_list_);
  pthread_mutex_unlock(&pool_mutex);
  TBSYS_LOG(DEBUG, "release conn %p", conn);
  TBSYS_LOG(DEBUG, "free list has %d", get_list_size(&conn->pool_->conn_list_.free_conn_list_));
  TBSYS_LOG(DEBUG, "used list has %d", get_list_size(&conn->pool_->conn_list_.used_conn_list_));
  return ret;
}
