...
namespace oceanbase
{
  namespace common
  {
    ...
    inline __attribute__((always_inline)) int ObRow::raw_set_cell(const int64_t cell_idx, const ObObj &cell)
    {
      int ret = OB_SUCCESS;

      if (OB_UNLIKELY(NULL == row_desc_))
      {
        ret = OB_NOT_INIT;
        TBSYS_LOG(ERROR, "row_desc_ is NULL");
      }
      else if (OB_UNLIKELY(row_desc_->get_column_num() <= cell_idx))
      {
        ret = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "invalid cell_idx=%ld cells_count=%ld", cell_idx, row_desc_->get_column_num());
      }
      else if (OB_UNLIKELY(OB_SUCCESS != (ret = raw_row_.set_cell(cell_idx, cell))))
      {
    ...
    inline int ObRow::get_rowkey(const ObRowkey *&rowkey) const
    {
      int ret = OB_SUCCESS;
      if (NULL == row_desc_)
      {
