...
void ObRow::assign(const ObRow &other)
{
  ...
  if (NULL == other.row_desc_)
  {
    this->row_desc_ = NULL;
  }
  else
  {
    set_row_desc(*other.row_desc_);
  }
}
...
int ObRow::set_cell(const uint64_t table_id, const uint64_t column_id, const common::ObObj &cell)
{
  ...
  if (NULL == row_desc_
      || OB_INVALID_INDEX == (cell_idx = row_desc_->get_idx(table_id, column_id)))
  {
    TBSYS_LOG(WARN, "failed to find cell, tid=%lu cid=%lu row_desc=%s",
        table_id, column_id, (NULL == row_desc_) ? "NULL" : to_cstring(*row_desc_));
    ret = OB_INVALID_ARGUMENT;
  }
