  ...
  namespace common
  {
    class ObRowStore
    {
      ...
      public:
        ...
        int get_last_stored_row(const StoredRow *&stored_row) const;
      ...
      private:
        ...
        int add_row(const ObRow &row, const StoredRow *&stored_row, int64_t &cur_size_counter);

        // @return OB_SIZE_OVERFLOW if buffer not enough
        int append_row(const ObRow &row, BlockInfo &block, StoredRow &stored_row);

        inline int append_extend_cell(ObCompactCellWriter& cell_writer, const ObObj& cell);
      ...
      private:
        ...
        int64_t last_prev_row_store_size_;
        int64_t last_row_store_size_;
