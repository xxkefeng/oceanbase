...
int64_t ObUpsList::to_string(char* buffer, const int64_t length) const
{
  int64_t pos = 0;
  for (int32_t i = 0; i < ups_count_; ++i)
  {
    databuff_printf(buffer, length, pos, "ups[%d]:", i);
    ups_array_[i].addr_.to_string(buffer+pos, length-pos);
    databuff_printf(buffer, length, pos, "(%d %hhd %hhd) ", 
        ups_array_[i].inner_port_, 
