...
void ObRootTable2::dump(common::ObNewScanner &scanner, const ObChunkServerManager &cs_mgr) const
{
  bool is_req_fullfill = true;
  int64_t row_count = 0;
  ObRowDesc desc;
  ObRow row;
  ObObj table_id;
  ...
  char start_key_buf[OB_MAX_VARCHAR_LENGTH];
  char end_key_buf[OB_MAX_VARCHAR_LENGTH];
  char cs_buf[3][OB_IP_STR_BUFF];
  int32_t start_key_buf_len = 0;
  int32_t end_key_buf_len = 0;
  ObObj row_count_obj;
  ObObj occupy_size_obj;
  ObObj crc_obj;
  ObObj svr_info_obj;
  ObString no_cs_str = ObString::make_string("NULL");
  ObString cs_str;

  // table id, start_key, end_key, row_count, occupy, crc, rep0, rep1, rep2
  for (int desc_idx = 0; desc_idx < 9; desc_idx++)
  {
    desc.add_column_desc(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + desc_idx);
  }
  desc.set_rowkey_cell_count(3);
  ...
  {
    tablet = tablet_info_manager_->get_tablet_info(it->tablet_info_index_);
    if (NULL == tablet)
    {
      TBSYS_LOG(ERROR, "no tablet info found in tablet_info_manager_ with tablet_index=%d", it->tablet_info_index_);
      break;
    }
    table_id.set_int(tablet->range_.table_id_);
    row_count_obj.set_int(tablet->row_count_);
    occupy_size_obj.set_int(tablet->occupy_size_);
    crc_obj.set_int(static_cast<int64_t>(tablet->crc_sum_));
    start_key_buf_len = static_cast<int32_t>(tablet->range_.start_key_.to_string(start_key_buf, OB_MAX_VARCHAR_LENGTH));
    end_key_buf_len = static_cast<int32_t>(tablet->range_.end_key_.to_string(end_key_buf, OB_MAX_VARCHAR_LENGTH));
    start_key_str.assign(start_key_buf, start_key_buf_len);
    end_key_str.assign(end_key_buf, end_key_buf_len);
    start_key.set_varchar(start_key_str);
    end_key.set_varchar(end_key_str);

    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 0, table_id);
    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 1, start_key);
    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 2, end_key);
    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 3, row_count_obj);
    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 4, occupy_size_obj);
    row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 5, crc_obj);
    for (int idx = 0; idx < common::OB_SAFE_COPY_COUNT && idx < 3; idx++)
    {
      const ObServerStatus* cs_status = NULL;
      if (it->server_info_indexes_[idx] >= 0)
      {
        cs_status = cs_mgr.get_server_status(it->server_info_indexes_[idx]);
      }
      if (NULL == cs_status)
      {
        svr_info_obj.set_varchar(no_cs_str);
      }
      else
      {
        ObServer server = cs_status->server_;
        server.set_port(cs_status->port_cs_);
        int64_t cs_buf_len = server.to_string(cs_buf[idx], OB_IP_STR_BUFF);
        cs_str.assign(cs_buf[idx], static_cast<int32_t>(cs_buf_len));
        svr_info_obj.set_varchar(cs_str);
      }
      row.set_cell(OB_ROOT_TABLE_TID, OB_APP_MIN_COLUMN_ID + 6 + idx, svr_info_obj);
    }
    if (OB_SUCCESS != scanner.add_row(row))
    {
      TBSYS_LOG(WARN, "root table info is not fully returned to client. row_count=%ld", row_count);
      break;
    }
    row_count++;
  }
  scanner.set_is_req_fullfilled(is_req_fullfill, row_count);
  return;
}
...
int ObRootTable2::get_table_tablet_info(
    const uint64_t table_id, int64_t& tablet_count, 
    int64_t& row_count, int64_t& data_size) const
{
  int ret = OB_SUCCESS;
  ...
  if (NULL == tablet_info_manager_)
  {
    TBSYS_LOG(WARN, "tablet_info_manager_ is NULL, can't get table tablet info");
    ret = OB_NOT_INIT;
  }
  else if (OB_SUCCESS != (ret = find_range(search_range, first, last)))
  {
    TBSYS_LOG(WARN, "cann't find this range %s, ret=%d",
              to_cstring(search_range), ret);
  }
  else
  {
    for (const_iterator it = first; it <= last; ++it)
    {
      if (NULL == (tablet_info = get_tablet_info(it)))
      {
        TBSYS_LOG(WARN, "root table get NULL tablet info");
        ret = OB_ERROR;
        break;
      }
      else
...
int ObRootTable2::create_table(const ObTabletInfo &tablet, const ObArray<int32_t> &chunkservers, const int64_t tablet_version, bool cs_create)
{
  int ret = OB_ERROR;
  ...
  if (tablet_info_manager_ != NULL)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for (int32_t i = 0; i < chunkservers.count() && i < OB_SAFE_COPY_COUNT; i++)
      {
        ...
        if (cs_create)
        {
          meta.tablet_status_[i] = common::TABLET_AVAILABLE;
        }
        else 
        {
          meta.tablet_status_[i] = common::TABLET_UNAVAILABLE;
        }
      }
    }
    if (OB_SUCCESS == ret)
...
bool ObRootTable2::check_table_all_tablets_available(const uint64_t table_id)
{
  bool ret = true;
  ...
  else
  {
    ...
    if (OB_SUCCESS != (err = find_range(search_range, first, last)))
    {
      TBSYS_LOG(WARN, "cann't find this range %s, ret=%d",
                to_cstring(search_range), ret);
      ret = false;
    ...
    {
      for (const_iterator it = first; it <= last && ret; ++it)
      {
        for (int32_t j = 0; j < OB_SAFE_COPY_COUNT; j++)
        {
          if (it->server_info_indexes_[j] != OB_INVALID_INDEX
              && common::TABLET_UNAVAILABLE == it->tablet_status_[j])
          {
            ret = false;
            break;
          }
