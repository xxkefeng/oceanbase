...
{ 
  namespace rootserver 
  {
    ...
    //only use to serialize and deserialize tablet version with tablet status, 
    //in order to be compatible with olu serialization format, this implementation
    //is very ugly
    //FIXME: find a better solution to do compatibility 
    union ObTabletStatusVersion
    {
      int64_t union_value_;
      struct
      {
        int8_t tablet_status_;
        int8_t reserved8_;
        int16_t reserved16_;
        int32_t tablet_version_;
      };
    };
    ...
    struct ObRootMeta2 
    {
      ...
      mutable int32_t tablet_status_[common::OB_SAFE_COPY_COUNT];  
