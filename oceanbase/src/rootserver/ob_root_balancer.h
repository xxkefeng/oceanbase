  ...
  namespace rootserver
  {
    ...
    class ObRootBalancer
    {
      ...
      private:
        ...
        int send_msg_migrate(const common::ObServer &src, const common::ObServer &dest, const common::ObNewRange& range, bool keep_src);
      ...
      private:
        int create_table(const uint64_t table_id);
        bool check_not_ini_table(const uint64_t table_id) const;
        void check_table_rereplication(const uint64_t table_id, const int64_t avg_count, const int64_t cs_num,
            bool & table_found, bool & scan_next_table);
