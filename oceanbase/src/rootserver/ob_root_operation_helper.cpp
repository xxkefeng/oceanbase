...
namespace oceanbase
{
  namespace rootserver
  {
    ObRootOperationHelper::ObRootOperationHelper()
      :is_build_index_(false), process_(INIT), root_server_(NULL),
        ...
        const int64_t frozen_version)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        switch (type)
        {
          ...
          case INDEX_TABLE:
            {
              TBSYS_LOG(INFO, "start to build sample. for sample process");
              ret = index_process(frozen_version, schema_mgr, table_name_id);
              break;
            }
          default:
    ...
    int ObRootOperationHelper::check_report_tablet_process()
    {
      ...
      for (; it != server_manager_->end(); ++it)
      {
        if (it->bypass_process_ != ObServerStatus::REPORTED)
        {
    ...
    int ObRootOperationHelper::check_bypass_process(OperationType &type)
    {
      int ret = OB_SUCCESS;
    ...
    int ObRootOperationHelper::check_index_process(OperationType &type)
    {
      int ret = OB_SUCCESS;
      ...
      //build index process
      //step1: build sample
      if (BUILD_SAMPLE == process_)
      {
        ret = check_build_sample_process(failed_count);
        if (failed_count > 0)
        {
          //TODO: notify all cs to stop build sample
        }
        if (OB_SUCCESS == ret)
        {
          if (failed_count > 0)
          {
            TBSYS_LOG(WARN, "failed to build sample, failed_cs_count=%ld", failed_count);
            bypass_data_.destroy_data();
            end_bypass_process();
          }
          else
          {
            TBSYS_LOG(INFO, "check build sample success. notity build index");
            if (OB_SUCCESS != (ret = root_server_->build_index_root_table()))
            {
              TBSYS_LOG(WARN, "fail to build root table with index table. ret=%d", ret);
              bypass_data_.destroy_data();
              end_bypass_process();
            }
            else if (OB_SUCCESS != (ret = request_cs_build_index(
               root_server_->get_frozen_version_for_cs_heartbeat(), index_builder_.get_build_index_info())))
            {
              TBSYS_LOG(WARN, "fail to request cs to build index. ret=%d", ret);
              bypass_data_.destroy_data();
              end_bypass_process();
            }
            else
            {
              TBSYS_LOG(INFO, "build sample ok, try to build index");
              start_time_us_ = tbsys::CTimeUtil::getTime();
              process_ = BUILD_INDEX;
            }
          }
        }
        else if (OB_INDEX_TIMEOUT == ret)
        {
          TBSYS_LOG(WARN, "check build sample timeout. end the build index process");
          bypass_data_.destroy_data();
          end_bypass_process();
        }
      }

      //step2: check build index process
      if (BUILD_INDEX == process_)
      {
        int32_t state = ObTableSchema::INDEX_ERROR;
        ret = check_build_index_process(failed_count);
        if (failed_count > 0)
        {
          //TODO: notify all cs to stop build index
        }
        if (OB_SUCCESS == ret)
        {
          if (failed_count > 0)
          {
            TBSYS_LOG(WARN, "%ld alive chunkservers fail to build index, need rebuild",
                      failed_count);
          }
          else
          {
            TBSYS_LOG(INFO, "check build index success. finish the all process");
            state = ObTableSchema::AVAILABLE;
          }
        }
        else if (OB_INDEX_TIMEOUT == ret)
        {
          TBSYS_LOG(WARN, "check build index timeout. end the build index process");
          state = ObTableSchema::INDEX_ERROR;
          bypass_data_.destroy_data();
          end_bypass_process();
        }
        if (OB_ERROR != ret 
            && OB_SUCCESS != root_server_->update_index_table_schema_state(
           index_builder_.get_build_index_info().index_table_id_, state))
        {
          TBSYS_LOG(WARN, "failed to update index table schema state, index_table_id=%lu, state=%d",
                    index_builder_.get_build_index_info().index_table_id_, state);
        }
        //TODO: check the status of slave cluster, then update internal table and schema
      }
    ...
    int ObRootOperationHelper::check_process(OperationType &type)
    {
      ...
      if (is_build_index_)
      {
        ret = check_index_process(type);
      }
      else 
      {
        ret = check_bypass_process(type);
      }
    ...
    int ObRootOperationHelper::cs_report_samples(
       const int32_t server_index, const ObNewScanner& sample_scanner)
    {
      ...
      if (BUILD_SAMPLE != process_)
      {
        TBSYS_LOG(WARN, "bypass not in report process. check the stat. process=%s", 
                  print_process());
        ret = OB_ERROR;
      }
      else
      {
        ret = index_builder_.report_samples(server_index, sample_scanner);
      }

      return ret;
    }

    int ObRootOperationHelper::check_build_sample_process(int64_t& failed_count)
    {
      int ret = OB_SUCCESS;
      ObChunkServerManager::iterator it = server_manager_->begin();
      failed_count = 0;
      ...
      {
        if (it->status_ != ObServerStatus::STATUS_DEAD
            && it->bypass_process_ != ObServerStatus::SAMPLED
            && it->bypass_process_ != ObServerStatus::FAILED)
        {
          TBSYS_LOG(TRACE, "cs have not finished building sample. server=%s", 
                    to_cstring(it->server_));
          ret = OB_ERROR;
          break;
        }
        else if (ObServerStatus::FAILED == it->bypass_process_)
        {
          //not check if the server is dead, if server dead, the bypass_process_
      ...
      {
        int64_t now = tbsys::CTimeUtil::getTime();
        int64_t timeout = config_->build_sample_time;
        if (start_time_us_ + config_->build_sample_time < now)
        {
          TBSYS_LOG(WARN, "cs build sample timeout. start_time=%ld, max_timeout=%ld, now=%ld",
              start_time_us_, timeout, now);
          ret = OB_INDEX_TIMEOUT;
        }
      }
    ...
    int ObRootOperationHelper::check_build_index_process(int64_t& failed_count)
    {
      int ret = OB_SUCCESS;
      ObChunkServerManager::iterator it = server_manager_->begin();
      failed_count = 0;
      ...
      {
        if (it->status_ != ObServerStatus::STATUS_DEAD
            && it->bypass_process_ != ObServerStatus::INDEXED
            && it->bypass_process_ != ObServerStatus::FAILED)
        {
          TBSYS_LOG(TRACE, "cs have not finished builing index. server=%s", 
                    to_cstring(it->server_));
          ret = OB_ERROR;
          break;
        }
        else if (ObServerStatus::STATUS_DEAD != it->status_
                 && ObServerStatus::FAILED == it->bypass_process_)
        {
          //alive cs build index failed
          ++failed_count;
        }
      }
      ...
      {
        int64_t now = tbsys::CTimeUtil::getTime();
        int64_t timeout = config_->build_index_time;
        if (start_time_us_ + config_->build_index_time < now)
        {
          TBSYS_LOG(WARN, "cs build index timeout. start_time=%ld, max_timeout=%ld, now=%ld",
              start_time_us_, timeout, now);
          ret = OB_INDEX_TIMEOUT;
        }
      }
    ...
    int ObRootOperationHelper::cs_build_sample_done(
       const bool is_succ, const int32_t cs_index, const int64_t index_table_id)
    {
      int ret = OB_SUCCESS;

      TBSYS_LOG(INFO, "cs_index=%d, cs=%s finish sample, index_table_id=%lu, is_succ=%d", 
                cs_index, to_cstring(server_manager_->get_cs(cs_index)), index_table_id, is_succ);
      ObServerStatus* status = server_manager_->get_server_status(cs_index);
      if (NULL == status)
      {
        TBSYS_LOG(WARN, "invalid status, status=%p, cs_index=%d", status, cs_index);
      ...
      else
      {
        if (!is_succ)
        {
          TBSYS_LOG(WARN, "cs_index=%d, cs=%s fail to sample.", 
                    cs_index, to_cstring(server_manager_->get_cs(cs_index)));
          status->bypass_process_ = ObServerStatus::FAILED;
        }
        else
        {
          status->bypass_process_ = ObServerStatus::SAMPLED;
        }
      }
    ...
    int ObRootOperationHelper::cs_build_index_done(
       const bool is_succ, const int32_t cs_index, const int64_t index_table_id)
    {
      int ret = OB_SUCCESS;

      TBSYS_LOG(INFO, "cs_index=%d, cs=%s finish build index, index_table_id=%lu, is_succ=%d", 
                cs_index, to_cstring(server_manager_->get_cs(cs_index)), index_table_id, is_succ);
      ObServerStatus* status = server_manager_->get_server_status(cs_index);
      if (NULL == status)
      {
      ...
      else
      {
        if (!is_succ)
        {
          TBSYS_LOG(WARN, "cs_index=%d, cs=%s fail to build index.", 
                    cs_index, to_cstring(server_manager_->get_cs(cs_index)));
          status->bypass_process_ = ObServerStatus::FAILED;
        }
        else
        {
          status->bypass_process_ = ObServerStatus::INDEXED;
        }
      }
    ...
    ObIndexOperation& ObRootOperationHelper::get_index_builder()
    {
      return index_builder_;
    }

    const bool ObRootOperationHelper::is_build_index() const 
    { 
      return is_build_index_; 
    }

    int ObRootOperationHelper::index_process(const int64_t frozen_version, 
        const common::ObSchemaManagerV2 *schema_mgr, const ObBypassTaskInfo &sample_task)
    {
      int ret = OB_SUCCESS;

      if (frozen_version <= 0 || NULL == schema_mgr)
      {
        TBSYS_LOG(WARN, "invalid param, frozen_version=%ld, schema_mgr=%p",
                  frozen_version, schema_mgr);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = index_builder_.init_sample_table(
         schema_mgr, sample_task.get_build_index_info())))
      {
        TBSYS_LOG(WARN, "fail to init sample table. ret=%d", ret);
      }
      else if (sample_task.get_build_index_info().sample_count_ > 0)
      {
        if (OB_SUCCESS != (ret = request_cs_build_sample(
           frozen_version, sample_task.get_build_index_info())))
        {
          TBSYS_LOG(WARN, "fail to request cs build sample, ret=%d", ret);
        }
        else
        {
          TBSYS_LOG(INFO, "root server enter build sample process");
          process_ = BUILD_SAMPLE;
          is_build_index_ = true;
          start_time_us_ = tbsys::CTimeUtil::getTime();
        }
      }
      else
      {
        if (OB_SUCCESS != (ret = request_cs_build_index(
           frozen_version, sample_task.get_build_index_info())))
        {
          TBSYS_LOG(WARN, "fail to request cs build index, ret=%d", ret);
        }
        else
        {
          TBSYS_LOG(INFO, "root server enter build index process");
          process_ = BUILD_INDEX;
          is_build_index_ = true;
          start_time_us_ = tbsys::CTimeUtil::getTime();
        }
    ...
    int ObRootOperationHelper::request_cs_build_sample(const int64_t frozen_version,
        const ObBuildIndexInfo& sample_info)
    {
      int ret = OB_SUCCESS;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (; it != server_manager_->end(); ++it)
        {
          if(it->status_ != ObServerStatus::STATUS_DEAD)
          {
            if (OB_SUCCESS != (ret = rpc_stub_->request_cs_build_sample(it->server_, 
                config_->network_timeout, frozen_version, sample_info.data_table_id_, 
                sample_info.index_table_id_, sample_info.sample_count_)))
            {
              TBSYS_LOG(WARN, "fail to request cs to build sample. cs_addr=%s", 
                        it->server_.to_cstring());
            }
          }
          else
          {
    ...
    int ObRootOperationHelper::request_cs_build_index(const int64_t frozen_version,
        const ObBuildIndexInfo& index_info)
    {
      int ret = OB_SUCCESS;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (; it != server_manager_->end(); ++it)
        {
          if(it->status_ != ObServerStatus::STATUS_DEAD)
          {
            if (OB_SUCCESS != (ret = rpc_stub_->request_cs_build_index(it->server_, 
                config_->network_timeout, frozen_version, index_info.data_table_id_, 
                index_info.index_table_id_)))
            {
              TBSYS_LOG(WARN, "fail to request cs to build index. cs_addr=%s", 
                        it->server_.to_cstring());
            }
          }
          else
