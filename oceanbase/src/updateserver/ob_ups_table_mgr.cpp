...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    class ObUpsQueryAllocator : public ObArenaAllocator
    {
      public:
        ObUpsQueryAllocator():ObArenaAllocator(ObModIds::OB_UPS_QUERY) {}
    };
                              ...
                              const int64_t timeout)
    {
      ...
      const ObNewRange *scan_range = NULL;
      bool is_final_minor = false;
      ObResourcePool<ObRowIterAdaptor>::Guard res_guard(row_iter_adaptor_rp_);
      ObMultipleScanMerge *multiple_scan_merge = GET_TSI_MULT(ObMultipleScanMerge, TSI_UPS_SCAN_MERGE_1);

      if (NULL == multiple_scan_merge)
      {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        TBSYS_LOG(WARN, "fail to scan tsi multiple_scan_merge");
      }
      else
      {
        multiple_scan_merge->reuse();
        multiple_scan_merge->set_is_ups_row(true);
      }
        ...
        else
        {
          ...
          do
          {
            ...
            for (iter = table_list->begin(); iter != table_list->end(); iter++, index++)
            {
              ...
              const ObRowDesc *row_desc = scanner.get_default_row_desc();
              if (NULL == row_desc)
              {
                ret = OB_ERR_UNEXPECTED;
                TBSYS_LOG(WARN, "row desc must not be NULL");
                break;
              }
              else
              {
                ObRowIterAdaptor *iter_adaptor = row_iter_adaptor_rp_.get(res_guard);
                if (NULL == iter_adaptor)
                {
                  ret = OB_ALLOCATE_MEMORY_FAILED;
                  TBSYS_LOG(WARN, "fail to alloc iter adaptor");
                }
                else
                {
                  iter_adaptor->reset();
                  iter_adaptor->set_cell_iter(table_iter, *row_desc, false);
                  if (OB_SUCCESS != (ret = multiple_scan_merge->set_child((int32_t)child_idx, *(iter_adaptor))))
                  {
                    TBSYS_LOG(WARN, "fail to add child operator:ret[%d]", ret);
                    break;
                  }
                  else
                  {
            ...
            {
              int64_t row_count = 0;
              ret = add_to_scanner_(*multiple_scan_merge, scanner, row_count, start_time, timeout, scan_param.get_scan_size());
              FILL_TRACE_BUF(session_ctx.get_tlog_buffer(), "add to scanner scanner_size=%ld row_count=%ld ret=%d", scanner.get_size(), row_count, ret);
              if (OB_SIZE_OVERFLOW == ret)
              {
                if (row_count > 0)
                                  ...
                                  const sql::ObLockFlag lock_flag)
    {
      ...
      ObMultipleGetMerge *multiple_get_merge = GET_TSI_MULT(ObMultipleGetMerge, TSI_UPS_GET_MERGE_1);

      if (NULL == multiple_get_merge)
      {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        TBSYS_LOG(WARN, "fail to get tsi multiple_get_merge");
      }
      else
      {
        multiple_get_merge->reuse();
        multiple_get_merge->set_is_ups_row(true);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          for (iter = table_list.begin(), index = 0; iter != table_list.end(); iter++, index++)
          {
            ...
            else
            {
              ...
              else
              {
                iter_adaptor->reset();
                iter_adaptor->set_cell_iter(table_iter, *row_desc, false);
                if (OB_SUCCESS != (ret = multiple_get_merge->set_child((int32_t)child_idx, *(iter_adaptor))))
                {
                  TBSYS_LOG(WARN, "fail to add child operator:ret[%d]", ret);
                  break;
                }
          ...
          {
            int64_t row_count = 0;
            ret = add_to_scanner_(*multiple_get_merge, scanner, row_count, start_time, timeout);
            if (OB_SIZE_OVERFLOW == ret)
            {
              // return ret
            }
    ...
    int ObUpsTableMgr :: new_get(const ObGetParam &get_param, common::ObNewScanner &scanner, const int64_t start_time, const int64_t timeout)
    {
      int ret = OB_SUCCESS;
    ...
    int ObUpsTableMgr :: new_scan(const ObScanParam &scan_param, common::ObNewScanner &scanner, const int64_t start_time, const int64_t timeout)
    {
      int ret = OB_SUCCESS;
    ...
    int ObUpsTableMgr::add_to_scanner_(ObIterator& ups_merger, ObNewScanner& scanner, int64_t& row_count,
                                        const int64_t start_time, const int64_t timeout, const int64_t result_limit_size/* = UINT64_SIZE*/)
    {
      int err = OB_SUCCESS;
      row_count = 0;
      const bool is_ups_row = true;
      common::ObRowIterAdaptor row_iter(is_ups_row);
      const ObRow *row = NULL;
      const bool is_deep_copy = false;
      const ObRowDesc* row_desc = scanner.get_default_row_desc();
      ...
      {
        row_iter.set_cell_iter(&ups_merger, *row_desc, is_deep_copy);
      }

      while (OB_SUCCESS == err)
      {
        err = row_iter.get_next_row(row);
        if (OB_ITER_END == err)
        {
          break;
        }
        ...
        {
          TBSYS_LOG(WARN, "fail to get next row:ret[%d]", err);
          break;
        }

        if (OB_SUCCESS != (err = scanner.add_row(*row)))
        {
          TBSYS_LOG(WARN, "fail to add row to scanner:ret[%d]", err);
          break;
        }
        else
        {
        ...
        if (0 < result_limit_size && result_limit_size <= scanner.get_size())
        {
          err = OB_SIZE_OVERFLOW;
          break;
        }
      ...
      if (OB_PROCESS_TIMEOUT == err || OB_SIZE_OVERFLOW == err)
      {
        int ret = OB_SUCCESS;
        if (OB_SUCCESS != (ret = scanner.set_last_row_key_from_row_store()))
        {
          err = ret;
          TBSYS_LOG(WARN, "fail to set last rowkey:ret[%d]", ret);
        }
      }
    ...
    int ObUpsTableMgr::add_to_scanner_(ObMultipleMerge &multiple_merge, ObNewScanner& scanner, int64_t& row_count,
                                        const int64_t start_time, const int64_t timeout, const int64_t result_limit_size/* = UINT64_SIZE*/)
    {
      int err = OB_SUCCESS;
      row_count = 0;
      ...
      if (multiple_merge.get_child_num() >= 1)
      {
        if (OB_SUCCESS != (err = multiple_merge.open()))
        {
          TBSYS_LOG(WARN, "fail to open multiple_merge:err[%d]", err);
        }

        while (OB_SUCCESS == err)
        {
          err = multiple_merge.get_next_row(row);
          if (OB_ITER_END == err)
          {
            break;
          }
        ...
        int tmp_err = multiple_merge.close();
        if (OB_SUCCESS != tmp_err)
        {
          err = tmp_err;
          TBSYS_LOG(WARN, "fail to close multiple_merge:err[%d]", err);
        }
      }
      return err;
    }
