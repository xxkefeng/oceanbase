          ...
          for (iter = schema_mgr.column_begin(); iter != schema_mgr.column_end(); iter++)
          {
            if (NULL != iter)
            {
              if (iter->get_table_id() != cur_table_id)
              {
                ...
                {
                  fprintf(fd, "[TABLE_SCHEMA] table_id=%lu table_load_type=%d table_name=%s split_pos=%d rowkey_max_length=%d\n",
                          iter->get_table_id(), table_schema->get_table_load_type(), table_schema->get_table_name(),
