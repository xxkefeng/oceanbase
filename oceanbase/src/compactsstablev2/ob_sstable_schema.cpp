...
namespace oceanbase
{
  namespace compactsstablev2
  {
    ...
    int ObSSTableSchema::get_sub_schema(const uint64_t table_id, ObSSTableSchema& schema)
    {
      int ret = OB_SUCCESS;
      ...
      else if (NULL == (def = get_table_schema(table_id, size)))
      {
        TBSYS_LOG(WARN, "get table schema error: table_id=[%lu]", table_id);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          if (OB_SUCCESS != (ret = schema.add_column_def(def[i])))
          {
            TBSYS_LOG(WARN, "schema add column desc error: ret=[%d], def=[%s]", ret, to_cstring(def[i]));
            break;
          }
        }
      }
        ...
        int64_t& size) const
    {
      ...
      else if (hash_init_)
      {
        ...
        {
          TBSYS_LOG(WARN, "can not go there");
        }
      }
      else
      {
        ...
        int64_t& size) const
    {
      ...
      if (!hash_init_)
      {
        TBSYS_LOG(WARN, "hash table is not init");
        inner_ret = OB_ERROR;
      }
      ...
      //rowkey column array
      if (OB_SUCCESS == inner_ret)
      {
        key.is_rowkey_ = true;
        if (-1 == (hash_get = table_schema_hashmap_.get(key, value)))
        {
          TBSYS_LOG(WARN, "table schema hashmap get key fail: key.table_id=[%lu], key.is_rowkey_=true", key.table_id_);
          inner_ret = OB_ERROR;
        }
        else if (hash::HASH_NOT_EXIST == hash_get)
        {
          TBSYS_LOG(INFO, "the hash get value of the key is empty: key.table_id=[%lu], key.is_rowkey_=true", key.table_id_);
          size = 0;
          ret = NULL;
        }
        else if (hash::HASH_EXIST == hash_get)
        {
          ...
          ret = column_def_ + offset;
        }
      }

      //rowvalue column array
      if (OB_SUCCESS == inner_ret && hash::HASH_EXIST == hash_get)
      {
        key.is_rowkey_ = false;
        if (-1 == (hash_get = table_schema_hashmap_.get(key, value)))
        {
          TBSYS_LOG(WARN, "table schema hashmap get key fail: key.table_id=[%lu], key.is_rowkey_=[false]", key.table_id_);
          inner_ret = OB_ERROR;
        }
        else if (hash::HASH_NOT_EXIST == hash_get)
        {
          TBSYS_LOG(INFO, "the hash get value of the key is not exist: key.table_id=[%lu], key.is_rowkey_=[false]", key.table_id_);
        }
        else if (hash::HASH_EXIST == hash_get)
        {
          size += value.size_;
    ...
    int ObSSTableSchema::check_row(const uint64_t table_id, const ObRow& row)
    {
      int ret = OB_SUCCESS;
      const int64_t rowkey_obj_cnt = row.get_row_desc()->get_rowkey_cell_count();
      ...
      if (OB_SUCCESS == ret)
      {
        def = const_cast<ObSSTableSchemaColumnDef*>(get_table_schema(table_id, false, size));
        if (size != rowvalue_obj_cnt)
        {
          if (size != rowvalue_obj_cnt)
          {
            TBSYS_LOG(WARN, "get table schema error:table_id=%lu,size=%ld,"
                            "rowvalue_obj_cnt=%ld",
                table_id, size, rowvalue_obj_cnt);
            ret = OB_ERROR;
          }
        }
        else
        {
          ...
          {
            if (OB_SUCCESS != (ret = row.raw_get_cell(i + rowkey_obj_cnt, obj, t_table_id, column_id)))
            {
              TBSYS_LOG(WARN, "rowvalue raw get cell error:ret=%d,i=%ld", ret, i);
              break;
            }
            else
            {
