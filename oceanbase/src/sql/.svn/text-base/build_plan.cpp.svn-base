    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      ret = logical_plan->add_query(explain_stmt, &query_id);
      if (ret != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    ObTableOption& option)
{
  ...
  if (node)
  {
    OB_ASSERT(node->type_ == T_TABLE_OPTION_LIST && node->num_child_ >= 1);
    ParseNode *option_node = NULL;
    int32_t char_number = 0;
    ...
    for (int32_t i = 0; ret == OB_SUCCESS && i < num; i++)
    {
      ...
      {
        case T_EXPIRE_INFO:
          option.expire_info_.assign_ptr(
        ...
        case T_TABLET_MAX_SIZE:
          option.tablet_max_size_ = option_node->children_[0]->value_;
          break;
        case T_TABLET_BLOCK_SIZE:
          option.tablet_block_size_ = option_node->children_[0]->value_;
          break;
        case T_REPLICA_NUM:
          option.replica_num_ = static_cast<int32_t>(option_node->children_[0]->value_);
          break;
        case T_COMPRESS_METHOD:
          option.compress_method_.assign_ptr(
              const_cast<char*>(option_node->children_[0]->str_value_),
              static_cast<int32_t>(option_node->children_[0]->value_));
          break;
        case T_USE_BLOOM_FILTER:
          option.use_bloom_filter_ = option_node->children_[0]->value_ ? true : false;
          break;
        case T_CONSISTENT_MODE:
          if (option_node->value_ == 1)
            option.read_static_ = true;
          break;
        case T_CHARSET:
          char_number = get_char_number_from_name(option_node->children_[0]->str_value_);
          if (OB_INVALID_CHARACTER_NUM == char_number)
          {
            TBSYS_LOG(DEBUG, "invalid char number %d", char_number);
            snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                     "Unknown character set: '%s'", option_node->children_[0]->str_value_);
            ret = OB_ERR_PARSE_SQL;
          }
          else
          {
            option.character_set_ = char_number;
          }
          break;
        default:
          /* won't be here */
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      ret = logical_plan->add_query(create_table_stmt, &query_id);
      if (ret != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
  ...
  {
    ObTableOption option;
    if ((ret = resolve_table_options(result_plan, node->children_[3], option)) != OB_SUCCESS)
    {
      // PARSER_LOG("Resolve table options failed, ret=%d", ret);
    }
    else if ((ret = create_table_stmt->set_table_option(*result_plan, option)) != OB_SUCCESS)
    {
      PARSER_LOG("Set table options failed, ret=%d", ret);
    }
  }
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      if ((ret = logical_plan->add_query(drp_tab_stmt, &query_id)) != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
...
{
  OB_ASSERT(result_plan);
  OB_ASSERT(node && node->type_ == T_CREATE_INDEX && node->num_child_ == 5);
  int& ret = result_plan->err_stat_.err_code_ = OB_SUCCESS;
  ObCreateIndexStmt* crt_idx_stmt = NULL;
  if (OB_SUCCESS != (ret = prepare_resolve_stmt(result_plan, query_id, crt_idx_stmt)))
  {
  }
  else
  {
    crt_idx_stmt->set_name_pool(static_cast<ObStringBuf*>(result_plan->name_pool_));
    crt_idx_stmt->set_unique(node->value_ == 1);
    ParseNode *cur_node = NULL;
    if (ret == OB_SUCCESS)
    {
      OB_ASSERT(node->children_[1]);
      cur_node = node->children_[1];
      int32_t len = static_cast<int32_t>(cur_node->value_);
      ObString table_name(len, len, cur_node->str_value_);
      if ((ret = crt_idx_stmt->set_table_name(*result_plan, table_name)) != OB_SUCCESS)
      {
        //PARSER_LOG("Set table name failed, table_name=%.*s, ret=%d", 
    ...
    {
      OB_ASSERT(node->children_[0]);
      cur_node = node->children_[0];
      int32_t len = static_cast<int32_t>(cur_node->value_);
      ObString index_name(len, len, cur_node->str_value_);
      if ((ret = crt_idx_stmt->set_index_name(*result_plan, index_name)) != OB_SUCCESS)
      {
        //PARSER_LOG("Set index name failed, index_name=%.*s, ret=%d", 
        //          index_name.length(), index_name.ptr(), ret);
      }
    }
    if (ret == OB_SUCCESS)
    {
      OB_ASSERT(node->children_[2] && node->children_[2]->type_ == T_INDEX_COLUMN_LIST);
      for (int32_t i = 0; ret == OB_SUCCESS && i < node->children_[2]->num_child_; i++)
      {
        cur_node = node->children_[2]->children_[i];
        ObColumnSortItem sort_item;
        sort_item.column_name_.assign_ptr(
                                  const_cast<char*>(cur_node->children_[0]->str_value_),
                                  static_cast<int32_t>(cur_node->children_[0]->value_));
        if ((ret = crt_idx_stmt->add_sort_column(*result_plan, sort_item)) != OB_SUCCESS)
        {
          PARSER_LOG("Add index column failed, index_name=%.*s, ret=%d", 
                     sort_item.column_name_.length(), sort_item.column_name_.ptr(), ret);
          break;
        }
        if (!cur_node->children_[1] || cur_node->children_[0]->type_ == T_SORT_ASC)
        {
          sort_item.order_type_ = ObColumnSortItem::ASC;
        }
        else
        {
          sort_item.order_type_ = ObColumnSortItem::DESC;
        }
      }
    }
    if (ret == OB_SUCCESS && node->children_[3])
    {
      for (int32_t i = 0; ret == OB_SUCCESS && i < node->children_[3]->num_child_; i++)
      {
        cur_node = node->children_[3]->children_[i];
        int32_t len = static_cast<int32_t>(cur_node->value_);
        ObString column_name(len, len, cur_node->str_value_);
        if ((ret = crt_idx_stmt->add_storing_column(*result_plan, column_name)) != OB_SUCCESS)
        {
          PARSER_LOG("Add storing column failed, index_name=%.*s, ret=%d", 
    ...
    {
      ObTableOption idx_option;
      if ((ret = resolve_table_options(result_plan, node->children_[4], idx_option)) != OB_SUCCESS)
      {
        // PARSER_LOG("Resolve index options failed, ret=%d", ret);
      }
      else if ((ret = crt_idx_stmt->set_index_option(*result_plan, idx_option)) != OB_SUCCESS)
      {
        PARSER_LOG("Set index options failed, ret=%d", ret);
      }
    }
  }
  return ret;
...
{
  OB_ASSERT(result_plan);
  OB_ASSERT(node && node->type_ == T_DROP_INDEX && node->num_child_ == 1);
  int& ret = result_plan->err_stat_.err_code_ = OB_SUCCESS;
  ObDropIndexStmt* drp_idx_stmt = NULL;
  if (OB_SUCCESS != (ret = prepare_resolve_stmt(result_plan, query_id, drp_idx_stmt)))
  {
  }
  else
  {
    drp_idx_stmt->set_name_pool(static_cast<ObStringBuf*>(result_plan->name_pool_));
    ParseNode *tab_node = node->children_[0];
    OB_ASSERT(tab_node);
    int32_t len = static_cast<int32_t>(tab_node->value_);
    ObString index_name(len, len, tab_node->str_value_);
    if ((ret = drp_idx_stmt->set_index_name(*result_plan, index_name)) != OB_SUCCESS)
    {
      PARSER_LOG("Set index name faile, index_name=%.*s, ret=%d", 
                 index_name.length(), index_name.ptr(), ret);
    }
  }
  return ret;
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      switch (node->type_)
      {
        ...
        case T_SHOW_INDEXES:
          ...
          show_table_node = node->children_[0];
          condition_node = node->children_[1];
          show_stmt = new(show_stmt) ObShowStmt(name_pool, ObBasicStmt::T_SHOW_INDEXES);
          sys_table_name.str_value_ = OB_INDEXES_SHOW_TABLE_NAME;
          break;
      ...
      if (ret == OB_SUCCESS && (ret = logical_plan->add_query(show_stmt, &query_id)) != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    if (ret == OB_SUCCESS && (node->type_ == T_SHOW_COLUMNS
      || node->type_ == T_SHOW_CREATE_TABLE || node->type_ == T_SHOW_INDEXES))
    {
    ...
    if (ret == OB_SUCCESS && condition_node
      ...
      || node->type_ == T_SHOW_PARAMETERS || node->type_ == T_SHOW_INDEXES))
    {
