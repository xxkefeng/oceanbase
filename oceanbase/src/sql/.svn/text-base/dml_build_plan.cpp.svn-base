  ...
  bool sub_query_results_scalar)
{
  ...
  switch (node->type_)
  {
    ...
    {
      ObObj val;
      val.set_unknown(reinterpret_cast<const ObObj *>(logical_plan->inc_question_mark()));
      ObConstRawExpr *c_expr = NULL;
      if (CREATE_RAW_EXPR(c_expr, ObConstRawExpr, result_plan) == NULL)
        break;
      c_expr->set_expr_type(T_QUESTIONMARK);
      c_expr->set_result_type(ObUnknownType);
      c_expr->set_value(val);
      expr = c_expr;
      break;
    }
    ...
    case T_SELECT:
    {
      ...
      if ((ret = resolve_select_stmt(result_plan, node, query_id)) != OB_SUCCESS)
        break;
      else if ((ret = stmt->add_subquery_index(
                                 logical_plan->get_bit_index_by_qid(query_id)
                                 )) != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
            "Add sub-query index failed");
      }
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if ((ret = logical_plan->add_query(select_stmt, &query_id)) != OB_SUCCESS)
    {
       snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
      ...
      switch (hint_node->type_)
      {
        ...
        case T_READ_CONSISTENCY:
          if (hint_node->value_ == 1)
            query_hint.read_consistency_ = ObQueryHint::STRONG;
          else
            query_hint.read_consistency_ = ObQueryHint::WEAK;
          break;
        default:
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      ret = logical_plan->add_query(delete_stmt, &query_id);
      if (ret != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      ret = logical_plan->add_query(insert_stmt, &query_id);
      if (ret != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      ret = logical_plan->add_query(update_stmt, &query_id);
      if (ret != OB_SUCCESS)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
