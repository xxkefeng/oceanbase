...
namespace oceanbase
{
  namespace sql
  {
    ObSSTableBlockScanner::ObSSTableBlockScanner(const ObSimpleColumnIndexes& column_indexes)
      ...
      is_reverse_scan_(false), is_full_row_scan_(false), 
      ...
      if (OB_UNLIKELY(row_cursor_ < row_start_index_ || row_cursor_ > row_last_index_))
      {
        ...
        ret = OB_ERROR;
            ...
            reader_.get_row(static_cast<int>(sstable_data_store_style_),
              row_cursor_, is_full_row_scan_, query_column_indexes_, current_row_))) 
      {
    ...
    int ObSSTableBlockScanner::get_next_row(const ObRow *&row_value)
    {
      int ret = OB_SUCCESS;
      if (end_of_block())
      {
    ...
    int ObSSTableBlockScanner::initialize(const bool is_reverse_scan, 
        const bool is_full_row_scan, const int64_t store_style)
    {
