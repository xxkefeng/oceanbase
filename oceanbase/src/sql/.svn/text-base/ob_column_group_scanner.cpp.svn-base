...
namespace oceanbase
{
  namespace sql
  {
        ...
        const ObSSTableReader *sstable_reader)
    {
      ...
      sstable::ObSimpleColumnIndexesBuilder index_builder(table_id, *schema,
          rowkey_info_, &row_desc_);
      ...
      {
        iret = index_builder.build_full_row_index(current_scan_column_indexes_, group);
      }
      else
      {
        iret = index_builder.build_input_col_index(current_scan_column_indexes_, group,
            column_id_begin, column_id_size);
      }
