#ifndef OCEANBASE_SQL_SCAN_SIMPLE_PARAM_H_
#define OCEANBASE_SQL_SCAN_SIMPLE_PARAM_H_
...
namespace oceanbase
{
  namespace sql
  {
    class ObSqlScanSimpleParam : public ObSqlReadSimpleParam 
    {
    public:
      ObSqlScanSimpleParam();
      virtual ~ObSqlScanSimpleParam();

    public:
      int set_range(const ObNewRange& range, bool deep_copy_args = false);
      inline const ObNewRange &get_range() const { return data_range_; }
      inline ObNewRange &get_range() { return data_range_; }
      inline ObStringBuf* get_buffer_pool() { return &buffer_pool_; }
...
#endif /* end of include guard: OCEANBASE_SQL_SCAN_SIMPLE_PARAM_H_ */
