  ...
  namespace chunkserver
  {
      ...
      while(OB_SUCCESS == ret && !is_tablet_unchanged)
      {
        ...
        else if ( cell_ != NULL )
        {
          ...
          if ( OB_SUCCESS == ret && ROW_START == row_status)
          {
            ...
            row_.set_column_group_id(
                column_group_id == ObSSTableSchemaColumnDef::ROWKEY_COLUMN_GROUP_ID ? 
                0 : column_group_id);
          ...
          if (OB_SUCCESS == ret && ROW_GROWING == row_status)
          {
            // only rowkey columns, ignore cell values;
            if (ObSSTableSchemaColumnDef::ROWKEY_COLUMN_GROUP_ID == column_group_id )
            {
              continue;
            }
    ...
    int ObTabletMergerV1::merge(ObTablet *tablet,int64_t frozen_version)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (tablet->get_sstable_id().sstable_file_id_ > 0)
          sstable_id = tablet->get_sstable_id().sstable_file_id_;
        ...
        else if (column_group_num == 0)
        {
          // table has only rowkey columns;
          if (NULL == sstable_schema_.get_group_schema(
                  new_table_schema_->get_table_id(), 
                  ObSSTableSchemaColumnDef::ROWKEY_COLUMN_GROUP_ID, 
                  column_group_num))
          {
            TBSYS_LOG(ERROR, "table [%ld] has no columns.", 
                new_table_schema_->get_table_id());
            ret = OB_ERROR;
          }
          else
          {
            column_group_num = 1;
            column_group_ids[0] = ObSSTableSchemaColumnDef::ROWKEY_COLUMN_GROUP_ID;
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ret = update_meta(old_tablet_, tablet_array_);
        }
        else
        {
          ...
          {
            if (((*it)->get_sstable_id().sstable_file_id_ > 0))
            {
    ...
    int ObTabletMergerV1::create_hard_link_sstable(int64_t& sstable_size)
    {
      ...
      if (old_tablet_->get_sstable_id().sstable_file_id_ > 0)
      {
        ...
        if (OB_SUCCESS == ret)
        {
           ...
           */
          sstable_size = 1;
          old_sstable_id = old_tablet_->get_sstable_id().sstable_file_id_;
        ...
        else
        {
          if (is_tablet_unchanged)
          {
            ...
            if (OB_SUCCESS == ret && sstable_size > 0
                && old_tablet_->get_sstable_id().sstable_file_id_ > 0)
            {
              trailer = &(dynamic_cast<ObSSTableReader*>(old_tablet_->get_sstable_reader()))->get_trailer();
            }
          }
          else
          {
