...
#define LOG_CACHE_MEMORY_USAGE_E(header, manager)                       \
  do  {                                                                 \
    TBSYS_LOG(INFO, "%s mgr_status_ =%ld,"       \
        ...
        header, manager->mgr_status_,                             \
        ob_get_mod_memory_usage(ObModIds::OB_CS_TABLET_IMAGE),    \
        manager->get_block_index_cache().get_cache_mem_size(), \
        manager->get_block_cache().size());                \
  } while(0);
...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    : is_init_(false),
      ...
      scan_recycler_(*this),
      tablet_image_(fileinfo_cache_, disk_manager_, log_manager_.get_log_worker(), &log_manager_),
      config_(NULL),
      build_index_thread_(*this)
    {
    }
                              ...
                              const int64_t max_sstable_size)
    {
      ...
      else if (!is_init_)
      {
        ...
        if (OB_SUCCESS == err)
        {
          block_cache_.set_fileinfo_cache(fileinfo_cache_);
          err = block_cache_.init(block_cache_size);
          compact_block_cache_.set_fileinfo_cache(fileinfo_cache_);
          compact_block_cache_.init(block_cache_size);
        }
        if (OB_SUCCESS == err)
        {
          block_index_cache_.set_fileinfo_cache(fileinfo_cache_);
          err = block_index_cache_.init(block_index_cache_size);
        ...
        if(OB_SUCCESS == err)
        {
          int64_t log_file_max_size = THE_CHUNK_SERVER.get_config().max_commit_log_size;
          int64_t log_sync_type = THE_CHUNK_SERVER.get_config().commit_log_sync_type;
          const char* log_dir = THE_CHUNK_SERVER.get_config().commit_log_dir;
          err = log_manager_.init(this, log_dir, log_file_max_size, log_sync_type);
        }
    ...
    int ObTabletManager::start_build_index_thread()
    {
      return build_index_thread_.init();
    }
    ...
    ObBuildIndexThread& ObTabletManager::get_build_index_thread()
    {
      return build_index_thread_;
    }
                                        ...
                                        uint64_t & crc_sum)
    {
      ...
      if (OB_SUCCESS == rc && NULL != tablet)
      {
        num_file = tablet->get_sstable_id().sstable_file_id_ > 0 ? 1 : 0;
      ...
      if (OB_SUCCESS == rc && NULL != tablet && num_file > 0)
      {
        ...
        const ObSSTableId & sstable_id = tablet->get_sstable_id();
        rc = get_sstable_path(sstable_id, src_path[0], OB_MAX_FILE_NAME_LENGTH);
        if ( OB_SUCCESS != rc )
        {
        ...
        {// generate dest path
          ...
          if (OB_SUCCESS == rc)
          {
            int n = snprintf(dest_path[0], OB_MAX_FILE_NAME_LENGTH,
                                          ...
                                          const uint64_t crc_sum)
    {
      ...
      int64_t serving_version = get_last_not_merged_version();
          ...
          || (serving_version > 0 && tablet_version > serving_version + 1))
      {
        ...
        rc = OB_ERROR;
            ...
            tablet_image.acquire_tablet(
              range,  ObMultiVersionTabletImage::SCAN_FORWARD, 0, old_tablet)) )
      {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if ( OB_SUCCESS == rc )
        {
          bool for_create = get_last_not_merged_version() == 0 ? true : false;
        ...
        if ( OB_SUCCESS == rc && OB_SUCCESS == tablet_exist && NULL != old_tablet)
        {
          ...
          rc = tablet_image.set_tablet_merged(old_tablet);
        }
      }
    ...
    int ObTabletManager::merge_multi_tablets(ObTabletReportInfoList& tablet_list)
    {
      ...
      {
        err = multi_tablet_merger->merge_tablets(*this, tablet_list, get_last_not_merged_version());
        if (OB_SUCCESS != err)
        {
    ...
    int ObTabletManager::create_tablet(const ObNewRange& range, const int64_t data_version)
    {
      ...
      if (range.empty() || 0 >= data_version ||
          (get_last_not_merged_version() > 0 && data_version != get_last_not_merged_version()))
      {
        TBSYS_LOG(ERROR, "create_tablet error, input range is empty "
            "or data_version=%ld or != serving data version %ld",
            data_version, get_last_not_merged_version());
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObTabletManager::load_tablets()
    {
      int err = OB_SUCCESS;
      ...
      int32_t size = 0;
      const int32_t* disk_no_array = log_manager_.has_ckpt() ?
        log_manager_.get_disk_no_array(size) : disk_manager_.get_disk_no_array(size);
      ...
      else if (OB_SUCCESS != (err = log_manager_.replay_log()))
      {
        TBSYS_LOG(ERROR, "replay log error, ret=%d", err);
      }
      else
      {
        //set recovered, start commit log
        log_manager_.get_log_worker()->set_recovered();
    ...
    ObGetThreadContext*& ObTabletManager::get_cur_thread_get_context()
    {
    ...
    {
      int ret = OB_SUCCESS;
      ObGetThreadContext*& get_context = get_cur_thread_get_context();
    ...
    int ObTabletManager::gen_sstable_getter_context(const ObSqlGetParam &sql_get_param, const chunkserver::ObGetThreadContext* &context, int64_t &tablet_version, int16_t &sstable_version)
    {
      int err                           = OB_SUCCESS;
      int64_t row_size                  = sql_get_param.get_row_size();
      ObMultiVersionTabletImage& image  = get_serving_tablet_image();
      ...
      if (OB_SUCCESS == err)
      {
        ...
        err = acquire_tablet(ObGetParamDecorator<ObSqlGetParam>(sql_get_param),
                             image, &get_context->tablets_[0],
                             get_context->tablets_count_,
                             tablet_version, &(get_context->min_compactsstable_version_));
      ...
      if (OB_SUCCESS == err && 0 == get_context->tablets_count_)
      {
        err = OB_CS_TABLET_NOT_EXIST;
          ...
          || get_context->tablets_count_ > sql_get_param.get_row_size())
      {
        TBSYS_LOG(WARN, "failed to acquire tablet, tablets size=%ld, row size=%ld, err=%d",
                  get_context->tablets_count_, sql_get_param.get_row_size(), err);
        err = OB_ERROR;
      }
      else
      {
        ObGetThreadContext*& thread_get_context = get_cur_thread_get_context();
        if (get_context != NULL)
        {
          thread_get_context = get_context;
        }

        sstable_version = get_context->tablets_[0]->get_sstable_version();
        if(sstable_version < SSTableReader::COMPACT_SSTABLE_VERSION)
          err = init_sstable_getter_context(get_context);
        else
          err = init_compact_sstable_getter_context(get_context);
    ...
    int ObTabletManager::gen_sstable_getter_context(const ObSqlGetSimpleParam &sql_get_param, const chunkserver::ObGetThreadContext* &context, int64_t &tablet_version, int16_t &sstable_version)
    {
      int err                           = OB_SUCCESS;
      int64_t row_size                  = sql_get_param.get_row_size();
      ObMultiVersionTabletImage& image  = get_serving_tablet_image();
      ...
      {
        get_context->tablets_count_ = OB_MAX_GET_ROW_NUMBER;
        err = acquire_tablet(ObGetParamDecorator<ObSqlGetSimpleParam>(sql_get_param),
                             image, &get_context->tablets_[0],
                             get_context->tablets_count_,
                             tablet_version, &(get_context->min_compactsstable_version_));
        FILL_TRACE_LOG("acquire_tablet:tablets_count_:%ld,version:%ld,err=%d",
    ...
    int64_t ObTabletManager::get_last_not_merged_version(void) const
    {
      return tablet_image_.get_last_not_merged_version();
    }
      ...
      else
      {
        while (OB_SUCCESS == err)
        {
          while (OB_SUCCESS == err && num > 0)
          {
            ...
            else if (tablet->get_data_version() != image.get_last_not_merged_version())
            {
            ...
            else
            {
              ...
              TBSYS_LOG(INFO, "add tablet [%ld] <%s>, row count:[%ld], size:[%ld], "
                  "crc:[%ld] version:[%ld] sequence_num:[%ld] to report list",
                  OB_MAX_TABLET_LIST_NUMBER - num,
        ...
        const ObTablet* tablet, ObSSTableScanner& sstable_scanner)
    {
      ...
      else if (OB_SUCCESS != (err = tablet->get_sstable_reader(sstable_reader)) )
      {
        TBSYS_LOG(WARN, "find_sstable err=%d", err);
      }
      else
      {
        err = sstable_scanner.set_scan_param(scan_param, sstable_reader,
          get_block_cache(), get_block_index_cache());
        if (OB_SUCCESS != err)
        {
    ...
    template <typename T>
    int ObTabletManager::acquire_tablet(const T &param, ObMultiVersionTabletImage& image,
                                        ...
                                        int64_t* compactsstable_version /*=NULL*/)
    {
      ...
      int64_t row_size = param.get_row_size();
      int64_t query_version = param.get_query_version();
      ObNewRange range;

      *compactsstable_version = 0;
      ...
      else
      {
        for (i = 0; i < row_size && OB_SUCCESS == err; i++)
        {
          ...
          range.table_id_ = param.get_table_id(i);
          range.start_key_ = param.get_rowkey(i);
          range.end_key_ = param.get_rowkey(i);
          range.border_flag_.set_inclusive_start();
          range.border_flag_.set_inclusive_end();
          err = image.acquire_tablet(range, ObMultiVersionTabletImage::SCAN_FORWARD,
              query_version, tablets[i]);
          if (OB_SUCCESS != err)
          {
      ...
      if (0 == size || 0 == cur_tablet_ver)
      {
         ...
         */
        cur_tablet_ver = get_last_not_merged_version();
      }
    ...
    int ObTabletManager::init_compact_sstable_getter_context(ObGetThreadContext *get_context)
    {
      int err                   = OB_SUCCESS;
      ...
      if (NULL == get_context)
      {
        TBSYS_LOG(WARN, "thread local instance of sstable getter is NULL");
        err = OB_ERROR;
      }
      else if (get_context->tablets_count_ <= 0)
      {
        TBSYS_LOG(WARN, "invalid param, tablets size=%ld", get_context->tablets_count_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        for (int64_t i = 0; i < get_context->tablets_count_ && OB_SUCCESS == err; ++i)
        {
          if (NULL == get_context->tablets_[i])
          {
            get_context->compact_getter_.readers_[i] = NULL;
            continue;
          }

          err = get_context->tablets_[i]->get_sstable_reader(reader);
          if (OB_SUCCESS == err && NULL != reader && 0 != reader->get_row_count())
          {
            TBSYS_LOG(DEBUG, "find_sstable reader=%p", reader);
            get_context->compact_getter_.readers_[i] = reader;

          }
          else if (OB_SIZE_OVERFLOW == err)
          {
            TBSYS_LOG(WARN, "find sstable reader by rowkey return more than"
                      "one reader, tablet=%p", get_context->tablets_[i]);
            err = OB_ERROR;
            break;
          }
          else
      ...
      if (OB_SUCCESS == err)
      {
        get_context->compact_getter_.readers_count_ = get_context->tablets_count_;
        get_context->compact_getter_.block_index_cache_ = &get_compact_block_index_cache();
        get_context->compact_getter_.block_cache_ = &get_compact_block_cache();
        get_context->compact_getter_.row_cache_ = sstable_row_cache_;
      }
    ...
    int ObTabletManager::init_sstable_getter_context(ObGetThreadContext *get_context)
    {
      int err                   = OB_SUCCESS;
      ...
      else
      {
        for (int64_t i = 0; i < get_context->tablets_count_ && OB_SUCCESS == err; ++i)
        {
          ...
          err = get_context->tablets_[i]->get_sstable_reader(reader);
          if (OB_SUCCESS == err && NULL != reader && !reader->empty())
          {
            TBSYS_LOG(DEBUG, "find_sstable reader=%p",
                      reader);
      ...
      if (OB_SUCCESS == err)
      {
        get_context->readers_count_ = get_context->tablets_count_;
        get_context->block_index_cache_ = &get_block_index_cache();
        get_context->block_cache_ = &get_block_cache();
        get_context->row_cache_ = sstable_row_cache_;
      }
    ...
    int ObTabletManager::send_tablet_report(const ObTabletReportInfoList& tablets, bool has_more)
    {
      ...
      RPC_RETRY_WAIT(is_init_,  retry_times, err,
          CS_RPC_CALL_RS(report_tablets, THE_CHUNK_SERVER.get_self(), tablets, get_last_not_merged_version(), has_more));

      return err;
    }

    int ObTabletManager::delete_tablet_on_rootserver(ObTablet* delete_tablets[], const int32_t size)
    {
      int ret = OB_SUCCESS;
    ...
    int ObTabletManager::uninstall_disk(int32_t disk_no)
    {
      ...
      //set the disk unvalible, so create tablet will not choose the disk
      disk_manager_.set_disk_status(disk_no, DISK_ERROR);

      //later will delete all the tablet on the disk_no,
      //but the tablet image idx on the disk_no, will not delete
      //just delete the tablet in memory
      //so do checkpoint
      if(OB_SUCCESS != (ret = sync_all_tablet_images()))
      {
        TBSYS_LOG(WARN, "sync all tablet images, ret=%d", ret);
      }

      //choose all the tablets locate the disk, then report the root server
      //so query the tablets locate the disk will not come
      ObTablet* tablet = NULL;

      ObMultiVersionTabletImage& image = get_serving_tablet_image();
      if(OB_SUCCESS == ret) ret = image.begin_scan_tablets();
      if(OB_ITER_END ==  ret)
      {
        image.end_scan_tablets();
      }
      ...
      else
      {
        while (OB_SUCCESS == ret)
        {
          ...
          else if(disk_no != tablet->get_disk_no())
          {
            image.release_tablet(tablet);
            continue;
          }
          ...
          {
            TBSYS_LOG(INFO, "add tablet <%s> to delete list, will remove later", to_cstring(tablet->get_range()));

            //add the tablet to delete tablet list
            delete_tablet_list.push_back(tablet);
          }
        }
      ...
      if(OB_SUCCESS == ret && delete_tablet_list.size() > 0)
      {
        int32_t idx = 0;
        ObTablet *delete_tablets[ delete_tablet_list.size() ];

        for(ObVector<ObTablet *>::iterator it = delete_tablet_list.begin(); it != delete_tablet_list.end(); ++it)
        {
          delete_tablets[idx++] = (*it);
        }

        //delete tablet on rootserver
        if(OB_SUCCESS != (ret = delete_tablet_on_rootserver(delete_tablets, delete_tablet_list.size())))
        {
          TBSYS_LOG(ERROR, "delete tablet report to rs fail, ret:%d tablet size:%d", ret, delete_tablet_list.size());
        }
        else
        {
          ObVector<ObTablet*>::iterator it = delete_tablet_list.begin();
          for(; it != delete_tablet_list.end() && OB_SUCCESS == ret; ++it)
          {
            //remove delete_tablet_list_ from the tablet image
            if(OB_SUCCESS != (ret = image.remove_tablet((*it)->get_range(), (*it)->get_data_version(), disk_no)))
            {
              TBSYS_LOG(ERROR, "remove tablet<%s> error, ret:%d", to_cstring((*it)->get_range()), ret);
            }
            else if(OB_SUCCESS != (ret = image.release_tablet((*it), false)))
            {
              TBSYS_LOG(ERROR, "failed to release tablet, tablet=%s", to_cstring((*it)->get_range()));
            }
          }
        }
      }
      ...
      //not flush the commit log
      if(OB_SUCCESS != (ret = image.flush_log(OB_LOG_CS_DISK_MAINTAIN, false)))
      {
        TBSYS_LOG(WARN, "flush log error, ret=%d", ret);
      }

      //free the memory vector
      delete_tablet_list.clear();
      is_disk_maintain_ = false;
      return ret;
    }
    ...
    int ObTabletManager::install_disk(const int32_t disk_no)
    {
      ...
      disk_manager_.set_disk_status(disk_no, DISK_NORMAL);
