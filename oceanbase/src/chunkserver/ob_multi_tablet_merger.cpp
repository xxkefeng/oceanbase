  ...
  namespace chunkserver 
  {
      ...
      const ObTabletReportInfoList& tablet_list)
    {
      ...
      for (int64_t i = 0; i < tablet_count && OB_SUCCESS == ret; ++i)
      {
        ...
        else if (NULL == (reader = dynamic_cast<ObSSTableReader*>(tablet->get_sstable_reader())))
        {
          tablet_infos[i].tablet_info_.range_.to_string(range_buf, sizeof(range_buf));
      ...
      for (int64_t i = 0; i < tablet_array_.get_array_index() && OB_SUCCESS == ret; ++i)
      {
        if (NULL != merge_tablets_[i])
        {
          ...
          if (OB_SUCCESS == ret)
          {
             ...
             */
            if(OB_SUCCESS != (ret = tablet_image.set_tablet_merged(merge_tablets_[i])))
            {
              TBSYS_LOG(ERROR, "failed to set tablet merged, ret=%d", ret);                }
            else if(OB_SUCCESS != (ret = tablet_image.release_tablet(merge_tablets_[i])))
            {
