  ...
  namespace chunkserver 
  {
    ...
    int ObTabletImage::destroy()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int32_t i = 0 ; i < tablet_count; ++i)
        {
          ...
          if(0 != tablet->dec_ref())
          {
              TBSYS_LOG(ERROR, "ObTable still been used not 0, "
                         "cannot destory..");
          }
      ...
      else
      {
        ...
        for (; it != tablet_list_.end(); ++it)
        {
          ...
          if (cur_table_id == table_id)
          {
            ...
            //delete from the tablet_list_, so dec the ref
            (*it)->dec_ref();
            if (hash_map_inited_)
            {
              hash_ret = sstable_to_tablet_map_.erase((*it)->get_sstable_id());
              if (hash::HASH_EXIST != hash_ret)
              {
                TBSYS_LOG(WARN, "failed to erase (sstabl_id, tablet) pair frome hash map, "
                    ...
                    (*it)->get_sstable_id().sstable_file_id_, (*it), hash_ret);
              }
            }
          }
    ...
    int ObTabletImage::delete_table(const uint64_t table_id, tbsys::CRWLock& lock)
    {
      ...
      if (OB_SUCCESS == ret && delete_table_tablet_list_.size() > 0)
      {
        ...
        else if(NULL != log_worker_ && OB_SUCCESS != (ret = log_worker_->delete_table(table_id)))
        {
          TBSYS_LOG(WARN, "delete_table: delete table redo log error, table_id=%lu, ret=%d", table_id, ret);
        }
      }
    ...
    void ObTabletImage::set_log_worker(ObChunkLogWorker* log_worker)
    {
    ...
    int ObTabletImage::add_tablet(ObTablet* tablet)
    {
      ...
      else if(tablet->get_sstable_id().sstable_file_id_ > 0)
      {
        // only after insert tablet into tablet list success, insert hash map
        hash_ret = sstable_to_tablet_map_.set(tablet->get_sstable_id(), tablet);
        if (hash::HASH_INSERT_SUCC != hash_ret)
        {
          TBSYS_LOG(WARN, "failed to insert (sstabl_id, tablet) pair into hash map, "
              "sstable_id=%lu, tablet=%p, hash_ret=%d",
              tablet->get_sstable_id().sstable_file_id_, tablet, hash_ret);
          ret = OB_ERROR;
        }
      }

      //whenever add fail or success, we will inc the ref, because later call release_tablet, will dec the ref
      tablet->inc_ref();
    ...
    int ObTabletImage::remove_sstable(ObTablet* tablet) const
    {
      ...
      {
        sstable_id = tablet->get_sstable_id();
    ...
    int ObTabletImage::remove_tablet(const common::ObNewRange& range, int32_t &disk_no)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret && NULL != tablet)
        {
          ...
          {
            {
              hash_ret = sstable_to_tablet_map_.erase(tablet->get_sstable_id());
              if (hash::HASH_EXIST != hash_ret)
              {
                TBSYS_LOG(WARN, "failed to erase (sstabl_id, tablet) pair frome hash map, "
                    "sstable_id=%lu, tablet=%p, hash_ret=%d",
          ...
          //not possible equal to 0. Generally, first acquire, then remove, last release, the ref_count then be 0
          if(0 == tablet->dec_ref())
          {
            remove_sstable(tablet);
            tablet->~ObTablet();
          ...
          //log the change
          if(log_worker_ != NULL && OB_SUCCESS != (ret = log_worker_->remove_tablet(data_version_, range, disk_no)))
          {
            TBSYS_LOG(ERROR, "remove_tablet : remove tablet redo log error, data_version=%ld, range=%s, disk_no=%d, ret=%d", data_version_, to_cstring(range), disk_no, ret);
          }
        }
      }
        ...
        const int32_t disk_no, const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < meta_header.tablet_count_; ++i)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            int64_t max_seq = tablet->get_sstable_file_seq();
    ...
    int ObTabletImage::prepare_write_meta(const int32_t disk_no, int64_t* size)
    {
      int ret = OB_SUCCESS;
      int64_t max_serialize_size = get_max_serialize(disk_no);
      ObThreadMetaWriter* writer = 
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if(NULL != size)
        {
          *size = writer->meta_data_size_ + writer->builder_.get_data_size() + writer->extend_.get_data_size();
        }
      }
    ...
    int ObTabletImage::write_meta(const char* data, const int64_t data_len, const char* idx_path, const int32_t disk_no)
    {
      int ret = OB_SUCCESS;
      char tmp_idx_file[OB_MAX_FILE_NAME_LENGTH];

      if (NULL == idx_path || strlen(idx_path) == 0 || NULL == data || 0 == data_len)
      {
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = get_tmp_meta_path(disk_no,
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != (ret = writer->appender_.open(fname, true, true, true)))
          {
            ...
            data, data_len, false) ))
          {
            TBSYS_LOG(ERROR, "write data buffer failed,%s.", strerror(errno));
          }
        }
    ...
    {
      TBSYS_LOG(INFO, "cur_iter_idx_=%d, memory usage=%ld", cur_iter_idx_, allocator_.total());
    ...
    ObMultiVersionTabletImage::ObMultiVersionTabletImage(IFileInfoMgr& fileinfo_cache)
      : newest_index_(0), last_not_merged_index_(-1), iterator_(*this), 
      fileinfo_cache_(fileinfo_cache), disk_manager_(NULL)
    {
      memset(image_tracker_, 0, sizeof(image_tracker_));
    }

    ObMultiVersionTabletImage::ObMultiVersionTabletImage(IFileInfoMgr& fileinfo_cache, ObDiskManager& disk_manager, ObChunkLogWorker* log_worker, ObChunkLogManager* log_manager)
      : newest_index_(0), last_not_merged_index_(-1), iterator_(*this), fileinfo_cache_(fileinfo_cache), disk_manager_(&disk_manager), log_worker_(log_worker), log_manager_(log_manager)
    {
      memset(image_tracker_, 0, sizeof(image_tracker_));
    }
        ...
        ObTablet* &tablet) const
    {
      ...
      if(0 == version)
      {
        if(has_tablet(newest_index_) &&
            OB_SUCCESS == (ret = image_tracker_[newest_index_]->acquire_tablet(range, scan_direction, tablet)))
        {

        }
        else if(has_tablet(eldest_index) &&
            OB_SUCCESS == (ret = image_tracker_[eldest_index]->acquire_tablet(range, scan_direction, tablet)))
        {
      ...
      {
        if(has_tablet(newest_index_) &&
            image_tracker_[newest_index_]->get_data_version() <= version &&
            (OB_SUCCESS == (ret = image_tracker_[newest_index_]->acquire_tablet(range, scan_direction, tablet))))
        {
        }
        else if(has_tablet(eldest_index) &&
            image_tracker_[eldest_index]->get_data_version() <= version &&
            (OB_SUCCESS == (ret = image_tracker_[eldest_index]->acquire_tablet(range, scan_direction, tablet))))
        {
        }
        else
      ...
      if(OB_SUCCESS != ret)
      {
        TBSYS_LOG(WARN, "cannot find range :%s ret:%d", to_cstring(range), ret);
      }
        ...
        ObTablet* &tablet) const
    {
      ...
      {
        if(has_tablet(newest_index_) &&
            OB_SUCCESS == (ret = image_tracker_[newest_index_]->acquire_tablet(sstable_id, tablet)))
        {

        }
        else if(has_tablet(eldest_index) &&
            OB_SUCCESS == (ret = image_tracker_[eldest_index]->acquire_tablet(sstable_id, tablet)))
        {
    ...
    int ObMultiVersionTabletImage::include_sstable(const ObSSTableId& sstable_id) const
    {
      ...
      else 
      {
        ...
        if(has_tablet(newest_index_) && OB_SUCCESS == (ret = image_tracker_[newest_index_]->include_sstable(sstable_id)))
        {

        }
        else if(has_tablet(eldest_index) && OB_SUCCESS == (ret = image_tracker_[eldest_index]->include_sstable(sstable_id)))
        {
    ...
    int ObMultiVersionTabletImage::flush_log(common::LogCommand cmd, bool flush)
    {
      int ret = OB_SUCCESS;
      if(NULL != log_worker_)
      {
        ret = log_worker_->flush_log(cmd, flush);
      }
      return ret;
    }

    int ObMultiVersionTabletImage::set_tablet_merged(ObTablet* tablet, const int status)
    {
      int ret = OB_SUCCESS;
      ...
      if(NULL != log_worker_)
      {
        ret = log_worker_->set_tablet_merged(tablet->get_data_version(), tablet->get_range(), status);
      }

      return ret;
    }

    int ObMultiVersionTabletImage::release_tablet(ObTablet* tablet, bool remove_sstable) const
    {
      int ret = OB_SUCCESS;
        ...
        const bool for_create /* = false */)
    {
      ...
      else 
      {
        ...
        else if (for_create && last_not_merged_index_ != newest_index_)
        {
          last_not_merged_index_ = newest_index_;
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        if(NULL != log_worker_ && OB_SUCCESS != (ret = log_worker_->add_tablet(tablet->get_data_version(), tablet, for_create)))
        {
          TBSYS_LOG(WARN, "add_tablet: add tablet redo log error, data_version=%ld,range=%s, ret=%d",
              tablet->get_data_version(), to_cstring(tablet->get_range()), ret);
        }
      }
        ...
        const bool load_sstable)
    {
      ...
      else 
      {
        ...
        for (int32_t i = 0; i < split_size && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            with_next_brother = 0;
            new_tablets[i]->set_with_next_brother(with_next_brother);
          }
          else
          {
            with_next_brother = 1;
          ...
          if(NULL != log_worker_ && OB_SUCCESS != (ret = log_worker_->set_with_next_brother(new_version, new_tablets[i]->get_range(), with_next_brother)))
          {
            TBSYS_LOG(ERROR, "set_with_next_brother: redo log error, data_version=%ld, sstable_file_id=%lu, with_next_brother=%d, ret=%d", new_version, new_tablets[i]->get_sstable_id().sstable_file_id_, with_next_brother, ret);
            break;
          }
          ...
          else if(NULL != log_worker_ && OB_SUCCESS != (ret = log_worker_->add_tablet(new_version, new_tablets[i])))
          {
            TBSYS_LOG(WARN, "add_tablet: add tablet commit log error, data_version=%ld,range=%s, ret=%d",
               new_version, to_cstring(new_tablets[i]->get_range()), ret);
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if(NULL != log_worker_)
          {
            ret = log_worker_->set_tablet_merged(old_tablet->get_data_version(), old_tablet->get_range());
          }
        }
      }
    ...
    int64_t ObMultiVersionTabletImage::get_eldest_index() const
    {
      ...
      else if(has_tablet(newest_index_))
      {
        index = newest_index_;
      }
    ...
    {
      int64_t eldest_version = 0;
      int64_t eldest_index = 0;

      {
        tbsys::CRLockGuard guard(lock_);
        eldest_index = get_eldest_index();

        if(eldest_index >= 0 && eldest_index < MAX_RESERVE_VERSION_COUNT &&  has_tablet(eldest_index))
        {
          eldest_version = image_tracker_[eldest_index]->get_data_version();
        }
      }

      return eldest_version;
    }

    int64_t ObMultiVersionTabletImage::get_last_not_merged_version() const
    {
      int64_t last_not_merged_version = 0;
      {
        tbsys::CRLockGuard guard(lock_);
        if(last_not_merged_index_ >= 0 && last_not_merged_index_ < MAX_RESERVE_VERSION_COUNT && has_tablet(last_not_merged_index_))
        {
          last_not_merged_version = image_tracker_[last_not_merged_index_]->get_data_version();
        }
      }

      return last_not_merged_version;
    }

    int64_t ObMultiVersionTabletImage::get_newest_version() const
    {
      int64_t newest_version = 0;
      {
        tbsys::CRLockGuard guard(lock_);
        if(has_tablet(newest_index_))
        {
          newest_version = image_tracker_[newest_index_]->get_data_version();
        }

        if(0 == newest_version && has_tablet((newest_index_ + 1) % MAX_RESERVE_VERSION_COUNT ))
        {
          newest_version = image_tracker_[(newest_index_ + 1) % MAX_RESERVE_VERSION_COUNT]->get_data_version();
        }
      }
    ...
    int ObMultiVersionTabletImage::prepare_tablet_image(const int64_t version, const bool destroy_exist)
    {
      ...
      else if (NULL == image_tracker_[index])
      {
        ...
        else
        {
          ...
          image_tracker_[index]->set_log_worker(log_worker_);
        }
      }
    ...
    int ObMultiVersionTabletImage::write(const char* data, const int64_t data_len, const int64_t version,
        const int32_t disk_no)
    {
      int ret = OB_SUCCESS;
      if (0 >= version || 0 >= disk_no || NULL == data || 0 == data_len)
      {
        TBSYS_LOG(WARN, "write image invalid argument, "
            "disk_no =%d <0 or version=%ld < 0", disk_no, version);
        ret = OB_INVALID_ARGUMENT;
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ret = get_image(version).write_meta(data, data_len, idx_path, disk_no);
        }
      }
      return ret;
    }
    ...
    int ObMultiVersionTabletImage::copy_meta_to_buf(common::ObBufferWriter* const wbuf)
    {
      int ret = OB_SUCCESS;
      ObThreadMetaWriter* writer =
        GET_TSI_MULT(ObThreadMetaWriter, TSI_CS_THEEAD_META_WRITER_1);
      ...
      else if(OB_SUCCESS != (ret = wbuf->write_varchar(writer->meta_buf_.get_buffer(), writer->meta_data_size_)))
      {
        TBSYS_LOG(ERROR, "copy tablet meta info error, ret:%d", ret);
      }
      else if(OB_SUCCESS != (ret = wbuf->write_varchar(writer->builder_.get_buf(), writer->builder_.get_data_size())))
      {
        TBSYS_LOG(ERROR, "copy rowkey meta info error, ret:%d", ret);
      }
      else if(OB_SUCCESS != (ret = wbuf->write_varchar(writer->extend_.get_buf(), writer->extend_.get_data_size())))
      {
        TBSYS_LOG(ERROR, "copy extend meta info error, ret:%d", ret);
      }
      ...
      const int32_t size)
    {
      ...
      else
      {
        ...
        common::ObBufferWriter eldest_buf[common::OB_MAX_DISK_NUMBER];
        common::ObBufferWriter newest_buf[common::OB_MAX_DISK_NUMBER];

        {
          ...
          {
            if(OB_SUCCESS != (ret = get_image(eldest_version).prepare_write_meta(disk_no_array[i], &buf_size)))
            {
              TBSYS_LOG(ERROR, "prepare write meta error, ret:%d", ret);
            }
            else
            {
              ...
              if(OB_SUCCESS == ret)
              {
                eldest_buf[i].assign(buf, buf_size);
                ret = copy_meta_to_buf(&(eldest_buf[i]));
              }
            }
          }
          ...
          if(newest_version != eldest_version) 
          {
            for (int32_t i = 0; i < size && OB_SUCCESS == ret; ++i)
            {
              if(OB_SUCCESS != (ret = get_image(newest_version).prepare_write_meta(disk_no_array[i], &buf_size)))
              {
                TBSYS_LOG(ERROR, "prepare write meta error, ret:%d", ret);
              }
              else
              {
                ...
                if(OB_SUCCESS == ret)
                {
                  newest_buf[i].assign(buf, buf_size);
                  ret = copy_meta_to_buf(&(newest_buf[i]));
                }
              }
            }
          }
          ...
          //switch the log file
          ret = log_manager_->switch_log_file(new_log_file_id);
          if(OB_SUCCESS != ret)
          {
        ...
        if(OB_SUCCESS == ret)
        {
          //write the tablet image buf to file
          for (int32_t i = 0; i < size; ++i)
          {
            ret = write(eldest_buf[i].buf(), eldest_buf[i].pos(), eldest_version, disk_no_array[i]);
            if(OB_SUCCESS != ret)
            {
          ...
          if(newest_version != eldest_version)
          {
            for (int32_t i = 0; i < size; ++i)
            {
              ret = write(newest_buf[i].buf(), newest_buf[i].pos(), newest_version, disk_no_array[i]);
              if(OB_SUCCESS != ret)
              {
                TBSYS_LOG(WARN, "write eldest meta info to disk=%d failed, version=%ld", 
              ...
              ob_free(newest_buf[i].buf());
              newest_buf[i].reset();
            }
          }

          //log check point
          log_manager_->do_check_point(new_log_file_id - 1, disk_no_array, size);
        }
      }
    ...
    int ObMultiVersionTabletImage::read(const int64_t version, const int32_t disk_no, const bool load_sstable)
    {
      int ret = OB_SUCCESS;
      char idx_path[OB_MAX_FILE_NAME_LENGTH];
        ...
        const int32_t size, const bool load_sstable)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        int eldest_merged = tablets_all_merged(eldest_index);

        if(OB_SUCCESS != eldest_merged)
        {
          last_not_merged_index_ = eldest_index;
        }
        else
        {
          last_not_merged_index_ = newest_index_;
    ...
    int ObMultiVersionTabletImage::finish_merge()
    {
      int ret = OB_SUCCESS;
      ...
      int64_t last_not_merged_version = 0;
      if(last_not_merged_index_ >= 0 &&
          last_not_merged_index_ < MAX_RESERVE_VERSION_COUNT &&
          has_tablet(last_not_merged_index_))
      {
        last_not_merged_version = image_tracker_[last_not_merged_index_]->data_version_;
      }
      ...
      else if (new_version <= last_not_merged_version)
      {
        TBSYS_LOG(WARN, "service version =%ld >= newest version =%ld, cannot upgrade.", 
            last_not_merged_version, new_version);
        ret = OB_ERROR;
      }
      else 
      {
    ...
    int ObMultiVersionTabletImage::Iterator::start()
    {
      ...
      cur_vi_ = image_.last_not_merged_index_;
      cur_ti_ = 0;
      start_vi_ = cur_vi_;
      end_vi_ = image_.last_not_merged_index_;
