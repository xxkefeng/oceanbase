...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    class ObTabletFactory
    {
      public:
        const static int32_t TABLET_COUNT_PER_MERGE = 256;
        ObTabletFactory();
        ~ObTabletFactory();
        int init(ObMultiVersionTabletImage *image , const int64_t frozen_version);
      public:
        int get_next_tablet(ObTablet* &tablet, bool & all_tablet_merged);
        int tablet_merge_done(ObTablet* tablet);
        // once DONOT fill cache from tablet image, MUST STOP add tablet 
        // in serving tablet image (by migrate in).
        inline bool pending_in_upgrade() const { return no_more_tablets_; }
        inline int64_t get_cache_list_size() const { return cache_list_.size(); }
      private:
        int fill_cache(int64_t &add_count);
        int get_tablet_from_cache_list(ObTablet* &tablet) ;
        void reset();
        void destroy();
      private:
        typedef common::ObList<ObTablet*> TabletList;
        typedef TabletList::iterator Iterator;
      private:
        ObMultiVersionTabletImage* image_;
        int64_t frozen_version_;
        TabletList cache_list_;
        TabletList merging_list_;
        // how many parallel merging tablet on every disk.
        int64_t disk_counter_[common::OB_MAX_DISK_NUMBER];
        // set true if we donot fetch tablet list from tablet image.
        bool no_more_tablets_;
    };
    ...
    class ObChunkMerge : public tbsys::CDefaultRunnable
    {
      public:
        ...
        {
          return (active_thread_num_ <= 1 && !round_start_);
        }
