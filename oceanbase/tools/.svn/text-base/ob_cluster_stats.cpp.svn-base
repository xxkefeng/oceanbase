...
namespace oceanbase
{
  namespace tools
  {
    ...
    {
      int ret = 0;
      ObRole server_type = current_.get_server_type();
      ...
      else if (server_type == OB_UPDATESERVER)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ret = update_stub.fetch_stats(current_);
        }
      }
    ...
    ObAppStats::ObAppStats(ObClientRpcStub &stub, const ObStatMod server_type, const char* config_file_name)
    ...
    int ObAppStats::sum_cluster_stats(const ObRole server_type, const common::ObArrayHelper<common::ObServer>& server_array)
    {
      ...
      for (int i = 0; i < server_array.get_array_index(); i++)
      {
        ...
        if (current_.begin(mod_) == current_.end(mod_))
          current_ = node_stats;
        else
          current_.add(node_stats);
      }
    ...
    int32_t ObAppStats::refresh()
    {
      ...
      current_.reset();
      current_.set_server_type(server_type);
