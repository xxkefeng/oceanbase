...
struct ObTabletExtendInfoV2
{
  ObTabletExtendInfoV2()
  {
    memset(this, 0, sizeof(ObTabletExtendInfoV2));
  }
  ...
  void build_extend_info_v3(ObTabletExtendInfo& info)
  {
    info.row_count_ = row_count_;
    info.occupy_size_ = occupy_size_;
    info.check_sum_ = check_sum_;
    info.row_checksum_ = row_checksum_;
    info.last_do_expire_version_ = last_do_expire_version_;
    info.sequence_num_ = sequence_num_;
    info.sstable_version_ = sstable_version_;
    info.reserved16_ = reserved16_;
    info.reserved32_ = reserved32_;
    memcpy(info.reserved_, reserved_, RESERVED_LEN * sizeof(int64_t));
  }
};

DEFINE_DESERIALIZE(ObTabletExtendInfoV2)
{
  int ret = OB_SUCCESS;
...
DEFINE_GET_SERIALIZE_SIZE(ObTabletExtendInfoV2)
{
    ...
    const int32_t disk_no, const char* buf, const int64_t data_len, int64_t& pos)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    ObTabletExtendInfoV2 extend_info;
    ObTablet * tablet = NULL;
    for (int64_t i = 0; i < meta_header.tablet_count_ && OB_SUCCESS == ret; ++i)
    {
      ...
      {
        ObTabletExtendInfo info;
        extend_info.build_extend_info_v3(info);
        tablet->set_extend_info(info);
      }
...
int main(int argc, char** argv)
{
  ...
  if (OB_SUCCESS != ret)
  {
    fprintf(stderr, "read idx :%s error:%d\n", idx_path, ret);
  }
  else 
  {
    if (dump) 
