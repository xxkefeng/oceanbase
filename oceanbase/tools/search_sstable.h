...
#include "sql/ob_noncg_sstable_scanner.h"
...
class RowScanOp
{
  ...
  public:
    ...
    int get_next_row(const oceanbase::common::ObRow *&row_value);
  private:
    ...
    oceanbase::sql::ObNoncgSSTableScanner scanner_;
    oceanbase::compactsstablev2::ObCompactSSTableScanner compact_scanner_;
    oceanbase::sql::ObRowIterator *iterator_;
...
class CellScanOp
{
  ...
  private:
    int build_row_desc(const oceanbase::common::ObScanParam& param, 
        const oceanbase::sstable::ObSSTableReader& reader);
    oceanbase::common::ModuleArena arena_;
    oceanbase::sstable::ObSSTableReader reader1_;
...
int scan_sstable_output_row(const CmdLineParam &clp)
{
  ...
  {
    while (OB_SUCCESS == (ret = scan_op.get_next_row(value)))
    {
      if (row_count >= clp.offset) 
      {
        if (row_count >= clp.offset + clp.limit)
        {
          break;
        }
        else if (clp.output) 
        {
          ...
          fprintf(stderr, "row[%ld]:[rkc:%ld], %s, value:%s\n", row_count, 
              value->get_row_desc()->get_rowkey_cell_count(), 
              NULL == key ? "nil": to_cstring(*key), to_cstring(*value));
        }
      }
