...
int RowScanOp::get_next_row(const ObRow *&row_value)
{
  int ret = OB_SUCCESS;
  if (NULL == iterator_)
  {
...
int CellScanOp::get_next_row(const ObRow *&row_value)
{
  return row_iter_.get_next_row(row_value);
}
...
{
  int opt = 0;
  const char* opt_string = "p:s:a:v:t:r:g:c:o:l:neyqh";
  ...
  while((opt = getopt_long(argc, argv, opt_string, longopts,NULL)) != -1)
  {
    switch (opt)
    {
      ...
      case 'c':
        ...
        break;
      case 'o':
        clp.offset = atoi(optarg);
        break;
      case 'l':
        clp.limit = atoi(optarg);
        break;
      case 'n':
        clp.output = false;
        break;
