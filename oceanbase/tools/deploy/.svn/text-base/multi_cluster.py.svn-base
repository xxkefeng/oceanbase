def MultiClusterAttr():
    ...
    start = 'all: obi start'
    collect_log = 'all: obi collect_log'
    update_local_bin = 'all: obi_master update_local_bin'
    obmysql = 'all: obi_master obmysql'
    rsync = 'all: obi rsync'
    force_stop = 'all: obi force_stop'
    print_schema = 'all: obi_master print_schema'
    return locals()
...
def MultiCluster(**cfg):
    ...
    obi_list[0].update(role='obi_master')
