...
prepare()
{
    ...
    if [ "$type" == all ]; then
        ...
        privilege_args="--mysql-user=$ob_user --mysql-password=$ob_password --oltp-schema-path=$base_dir/${case}.sql $simon_args"
...
start()
{
    ...
    if [ "$type" == all ]; then
        ...
        args="--test=oltp --oltp-table-size=$cs_rows --oltp-auto-inc=off --mysql-port=$merge_server_mysql_port --mysql-host=$merge_server_str --db-ps-mode=$ps_mode --ob=on --max-time=$max_seconds --max-requests=$max_requests --num-threads=$test_threads  --oltp-dist-type=uniform --oltp-skip-trx=on --oltp-test-mode=complex --oltp-range-size=$range_size --oltp-schema-path=$base_dir/${case}.sql"

        echo "Run test backend: LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH $base_dir/sysbench $privilege_args $args $simon_args --oltp-row-offset=0 run >$base_dir/perf_${client_idx}.log 2>&1 &"
        set -x
        env LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH nohup $base_dir/sysbench $privilege_args $args $simon_args --oltp-row-offset=0 run >$base_dir/perf_${client_idx}.log 2>&1 &
        set +x
        sleep 3
    else
        echo "operation[$type] not support!"
