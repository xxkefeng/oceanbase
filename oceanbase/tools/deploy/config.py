...
#tpl.update(ups_cfg='total_memory_limit=10G,trans_thread_num=14,io_thread_count=4')
...
# ob1 = OBI(tpl=dict((ups_cfg='disk_warn_threshold=100ms')), hosts=HostPool(2))
...
ob3 = OBI('OBI_MASTER', masters = ObCfg.default_hosts[:1], ct=CT('syschecker', hosts = ObCfg.default_hosts[:1]), tpl = dict(schema_template=read('syschecker/syschecker.schema')))
mysql = {
"ip" : "10.232.36.187",
"port" : "3306",
"user" : "sqltest",
...
ob6 = OBI(src='/home/qushan/oceanbase/branches/dev/oceanbase',tpl = dict(schema_template=read('ob_new_sql_test.schema')))
