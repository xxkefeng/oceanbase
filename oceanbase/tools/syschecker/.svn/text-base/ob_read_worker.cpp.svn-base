...
namespace oceanbase
{
  namespace syschecker
  {
        ...
        ObScanner& scanner)
    {
      ...
      get_param.reset();
      scanner.clear();
      get_param.set_is_result_cached(read_param.is_result_cached_);
        ...
        ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        scan_param.set_is_result_cached(read_param.is_result_cached_);
    ...
    void ObReadWorker::run(CThread* thread, void* arg)
    {
      ...
      {
        if (OB_SUCCESS != (client_.get_last_frozen_version(read_param->last_frozen_version_)))
        {
          TBSYS_LOG(WARN, "get_last_frozen_version error");
          return;
        }
        while (!_stop)
        {
    ...
    int ObReadWorker::fill_sql_scan_param(ObOpParam& read_param, sql::ObSqlScanParam& scan_param, common::ObRowDesc& row_desc)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        scan_param.set_data_version(read_param.last_frozen_version_);
    ...
    int ObReadWorker::fill_sql_get_param(ObOpParam& read_param, sql::ObSqlGetParam& get_param, common::ObRowDesc& row_desc)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        get_param.set_data_version(read_param.last_frozen_version_);
        get_param.set_is_result_cached(read_param.is_result_cached_);
        ...
        const bool is_get)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (is_get)
        {
          stat_.add_get_opt(1);
          stat_.add_get_cell(total_cell_count);
          stat_.add_get_row(row_count);
          stat_.add_get_cell_fail(read_cell_fail);
        }
        else
        {
          stat_.add_scan_opt(1);
          stat_.add_scan_row(row_count);
          stat_.add_scan_cell(total_cell_count);
          stat_.add_scan_cell_fail(read_cell_fail);
        }
      }
    ...
    int build_scan_param(
        ...
        const int64_t *query_column_array, 
        const int64_t query_column_size, 
        const int64_t query_data_version,
        sql::ObSqlScanParam& scan_param,
        common::ObRowDesc& row_desc)
    {
      ...
      else
      {
        ...
        scan_param.set_read_mode(oceanbase::common::ScanFlag::SYNCREAD);
