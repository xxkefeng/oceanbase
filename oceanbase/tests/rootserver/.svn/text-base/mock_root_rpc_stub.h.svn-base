...
namespace oceanbase {
  namespace rootserver {
    ...
    MOCK_METHOD3(request_cs_load_bypass_tablet,
        int(const common::ObServer& chunkserver, const common::ObTableImportInfoList &import_info, const int64_t timeout_us));
    MOCK_METHOD3(request_cs_delete_table, int(const common::ObServer& chunkserver, const uint64_t table_id, const int64_t timeout_us));

    int get_cs_merge_state(common::ObServer &chunkserver, int64_t &service_version,
        bool &is_merge_done, const int64_t timeout_us)
    {
      TBSYS_LOG(INFO, "chunkserver addr is %s", to_cstring(chunkserver));
      service_version = 2;
      is_merge_done = true;
      UNUSED(timeout_us);
      return OB_SUCCESS;
