...
TEST(SchemaTestV2, write_to_file)
{
  tbsys::CConfig c1;
  tbsys::CConfig c2;
  ObSchemaManagerV2 *schema_manager = new ObSchemaManagerV2();
  ASSERT_EQ(true, schema_manager->parse_from_file("./schema.ini", c1));
  schema_manager->write_to_file("./schema_bypass.ini");
  ObSchemaManagerV2 *schema_manager2 = new ObSchemaManagerV2();
  ASSERT_EQ(true, schema_manager2->parse_from_file("./schema_bypass.ini", c2));
  ASSERT_EQ(schema_manager->get_column_count(), schema_manager2->get_column_count());
  ASSERT_EQ(schema_manager->get_table_count(), schema_manager2->get_table_count());
  ASSERT_EQ(schema_manager->get_max_table_id(), schema_manager2->get_max_table_id());
  ASSERT_EQ(schema_manager->get_version(), schema_manager2->get_version());
  for (int i = 0; i < schema_manager->get_column_count(); i++)
  {
    const ObColumnSchemaV2* column = schema_manager->column_begin() + i;
    const ObColumnSchemaV2* column2 = schema_manager2->get_column_schema(column->get_table_id(), column->get_id());
    column->print_info();
    column2->print_info();
    ASSERT_EQ(true, *column == *column2);
  }
  for (int i = 0; i < schema_manager->get_table_count(); i++)
  {
    const ObTableSchema* table = schema_manager->table_begin() + i;
    const ObTableSchema* table2 = schema_manager2->get_table_schema(table->get_table_name());
    ASSERT_EQ(true, *table == *table2);
  }
}
...
TEST(SchemaTest, index)
{
  ObSchemaManagerV2 *schema_mgr = new ObSchemaManagerV2();
  ObTableSchema table_schema;
  table_schema.set_table_id(1);
  table_schema.set_table_name("table1");
  schema_mgr->add_table(table_schema);
  table_schema.set_table_id(2);
  table_schema.set_table_name("table2");
  schema_mgr->add_table(table_schema);

  ObTableSchema index;
  index.set_table_id(11);
  index.set_table_name("index11");
  index.set_table_type(ObTableSchema::INDEX);
  index.set_data_table_id(1);
  schema_mgr->add_table(index);
  index.set_table_id(12);
  index.set_table_name("index12");
  index.set_table_type(ObTableSchema::INDEX);
  index.set_data_table_id(1);
  schema_mgr->add_table(index);

  ASSERT_EQ(OB_SUCCESS, schema_mgr->cons_table_to_index_relation());

  uint64_t index_tid_array[128];
  int64_t size = 128;
  const ObTableSchema * tschema = schema_mgr->get_table_schema(1);
  tschema->get_index_tid_array(index_tid_array, size);
  ASSERT_EQ(2, size);
  for(int i = 0; i < 2; i ++)
  {
    ASSERT_TRUE(index_tid_array[i] == 11 || index_tid_array[i] == 12);
  }
  tschema = schema_mgr->get_table_schema(2);
  tschema->get_index_tid_array(index_tid_array, size);
  ASSERT_EQ(0, size);
}
