...
{ 
  namespace syschecker 
  {
    ...
    int ObSyschecker::get_cell_value(const ObObj& obj, 
                                     int64_t& value)
    {
      int ret             = OB_SUCCESS;
      float floatv        = 0.0;
      double doublev      = 0.0;
      ObObjType cell_type = obj.get_type();
      ObString varcharv;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        case ObFloatType:
          obj.get_float(floatv);
          value = static_cast<int64_t>(floatv);
          break;

        case ObDoubleType:
          obj.get_double(doublev);
          value = static_cast<int64_t>(doublev);
          break;

        case ObDateTimeType:
          obj.get_datetime(value);
          break;

        case ObPreciseDateTimeType:
          obj.get_precise_datetime(value);
          break;

        case ObVarcharType:
          obj.get_varchar(varcharv);
          value = strtoll(varcharv.ptr(), NULL, 10);
          break; 
        
        case ObExtendType:
          if (obj.get_ext() == ObActionFlag::OP_ROW_DOES_NOT_EXIST)
          {
    ...
    int ObSyschecker::read_rowkey_range(ObScanner& scanner)
    {
           ...
           iter++, index++)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          if (0 == index)
          {
            ret = get_cell_value(cell_info.value_, prefix);
            if (OB_SUCCESS == ret)
            {
          ...
          {
            ret = get_cell_value(cell_info.value_, suffix);
            if (OB_SUCCESS == ret)
            {
    ...
    int ObSyschecker::read_rowkey_range(const ObRowDesc& row_desc, ObNewScanner& scanner)
    {
      int ret         = OB_SUCCESS;
      ...
      int64_t prefix = 0;
      int64_t suffix = 0;
      if (OB_SUCCESS != (ret = scanner.get_next_row(current_row)))
      {
        TBSYS_LOG(ERROR, "read 0,0 at least return one row, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = current_row.raw_get_cell(0, cell, table_id, column_id)))
      {
        TBSYS_LOG(ERROR, "read_rowkey row 0 not exist, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = get_cell_value(*cell, prefix)))
      {
        TBSYS_LOG(ERROR, "read_rowkey row 0 not int, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = current_row.raw_get_cell(1, cell, table_id, column_id)))
      {
        TBSYS_LOG(ERROR, "read_rowkey row 1 not exist, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = get_cell_value(*cell, suffix)))
      {
        TBSYS_LOG(ERROR, "read_rowkey row 1 not int, ret=%d", ret);
      }
      else
      {
        rule_.add_cur_max_prefix(prefix + param_.get_syschecker_no());
    ...
    int ObSyschecker::init_rowkey_range_by_scan()
    {
      int ret                               = OB_SUCCESS;
      const ObTableSchema* wt_schema        = NULL;
      const ObColumnSchemaV2* column_schema = NULL;
      ...
      row_key.assign (rowkey_obj_array, MAX_SYSCHECKER_ROWKEY_COLUMN_COUNT);
      scan_range.table_id_ = syschecker_schema_.get_wt_schema()->get_table_id();
      scan_range.start_key_ = row_key;
      scan_range.end_key_ = row_key;
      ...
      if (NULL != wt_schema)
      {
        ...
        for (int64_t i = 0; i < column_size && query_size < 2 && OB_SUCCESS == ret; ++i)
        {
          ...
          if (NULL != &column_schema[i])
          {
            query_column_array[query_size++] = column_schema[i].get_id();
          }
          else
          {
            ret = OB_ERROR;
      ...
      if (OB_SUCCESS == ret)
      {
        ret = build_scan_param(scan_range, query_column_array, query_size, -2, scan_param, row_desc);
      }
      if (OB_SUCCESS == ret)
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ret = read_rowkey_range(row_desc, scanner);
      }
