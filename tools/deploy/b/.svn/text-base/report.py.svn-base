...
def load_single_data_dir(data_dir):
    ...
    try:
        tps, rt, sc, qps, qrt = query('select avg(tps),avg(rt),count(tps), avg(qps), avg(qrt) from t_client')
        tps, rt, sc, qps, qrt = float(tps) * int(meta.get('client_num')), float(rt) * int(meta.get('trans_size')), float(sc), float(qps) * int(meta.get('client_num')), float(qrt) * int(meta.get('trans_size'))
    except Exception,e:
        print e
        tps, rt, sc, qps, qrt = 0, 0, 0, 0, 0
    try:
    ...
    try:
        dml, = query('select avg(tps) from t_dml')
    except Exception, e:
        print e
        dml = 0
    img_dict = dict(('%s_img'%(i), make_img_html('%s/%s.png'%(spec, i))) for i in 'rt tps qrt qps dml batch disk net'.split())
    return dict(tps=tps, rt=rt, sc=sc, qps=qps, qrt=qrt, disk=disk, net=net, batch=batch, dml=dml,
                **dict_updated(meta, **img_dict))
    
def get_perf_data(data_dir):
    env = dict(test_meta=safe_read('%s/meta'%(data_dir)), log_dir=data_dir, plot_url='http://10.232.35.40:8001/home/yuanqi.xhf/ob.dev/tools/deploy/%s'%(data_dir))
...
def plot_all(spec_list):
    ...
    avg_columns, img_columns = 'spec dml tps rt qps qrt:spec dml_img rt_img tps_img'.split(':')
