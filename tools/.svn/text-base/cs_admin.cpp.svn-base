...
GFactory::GFactory() : sql_rpc_buffer_(2 << 20)
{
}
...
void GFactory::init_cmd_map()
{
  ...
  cmd_map_["scan"] = CMD_SCAN;
  cmd_map_["get"] = CMD_GET;
  cmd_map_["cell_scan"] = CMD_CELL_SCAN;
  cmd_map_["install_disk"] = CMD_INSTALL_DISK;
  cmd_map_["uninstall_disk"] = CMD_UNINSTALL_DISK;
  cmd_map_["cell_get"] = CMD_CELL_GET;
  cmd_map_["ups_scan"] = CMD_UPS_SCAN;
  cmd_map_["sstable_scan"] = CMD_SSTABLE_SCAN;
  cmd_map_["print_schema"] = CMD_PRINT_SCHEMA;
  cmd_map_["stop_build_index"] = CMD_STOP_BUILD_INDEX;
}
...
int GFactory::initialize(const ObServer& remote_server)
{
  ...
  ret = sql_rpc_stub_.init(&sql_rpc_buffer_, &client_.get_client_mgr());
  if (OB_SUCCESS != ret)
  {
    TBSYS_LOG(ERROR, "Init sql_rpc_stub_ failed, ret:%d", ret);
    return ret;
  }
...
int disk_maintain(const vector<string> &param, int8_t is_install)
{
  ...
  return GFactory::get_instance().get_rpc_stub().cs_disk_maintain(is_install, disk_no);
}
    ...
    )
{
  ...
  {
    server_stats = new ObServerStats(rpc_stub, mod);
  }
  else if (obj_type == ClusterStats)
  {
    server_stats = new ObClusterStats(rpc_stub, mod);
  }
  else if (obj_type == AppStats)
  {
    server_stats = new ObAppStats(rpc_stub, mod, config_file_name);
  }
  else
  {
    return NULL;
  ...
  if (NULL != table_filter_string)
  {
    ...
    {
      if (table_id_size > 1 || table_id_array[0] != -1)
      {
  ...
  if (mod == common::OB_MAX_MOD_NUMBER)
  {
...
{
  static const int32_t server_count = 8;
  static const char* server_name[server_count] = {"rs", "cs", "ms", "ups", "sql", "mysql", "common", "sstable"};
  int ret = 0;
  if (param.size() < 1)
  {
  ...
  ObServerStats* server_stats = create_stats_object(
      objtype,
      rpc_stub,
      static_cast<ObStatMod>(server_type),
      show_header,
      table_filter_string,
      index_filter_string,
      g_config_file_name
...
int scan(const vector<string> &param)
{
  int ret = OB_SUCCESS;
  // scan table_id range_str columns_str version
  //
  if (param.size() < 4) 
  {
    fprintf(stderr, "scan table_id range_str columns_str version [is_async_read] [is_result_cached]\n");
    return OB_ERROR;
  }

  QueryParam query_param;
  query_param.table_id = strtoll(param[0].c_str(), NULL, 10);
  query_param.scan_range = param[1].c_str();
  query_param.query_columns = param[2].c_str();
  query_param.version = strtoll(param[3].c_str(), NULL, 10);
  if (param.size() > 4) query_param.is_async_read = strtoll(param[4].c_str(), NULL, 10);
  if (param.size() > 5) query_param.is_result_cached = strtoll(param[5].c_str(), NULL, 10);

  ObSqlScanParam scan_param;
  ObNewScanner scanner;
  ObRowDesc row_desc;

  if (OB_SUCCESS != (ret = build_scan_param(query_param, scan_param, row_desc)))
  {
  }
  else if (OB_SUCCESS != (ret = GFactory::get_instance().get_rpc_stub().cs_sql_scan(scan_param, scanner)))
  {
  }
  else
  {
    dump_scanner(scanner, row_desc);
  }

  return ret;
}

int cell_scan(const vector<string> &param)
{
  int ret = OB_SUCCESS;
  // scan table_id range_str columns_str version
  //
  ...
  if (OB_SUCCESS != (ret = build_scan_param(query_param, scan_param)))
  {
    TBSYS_LOG(ERROR, "build_scan_param ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = GFactory::get_instance().get_rpc_stub().cs_scan(scan_param, scanner)))
  {
    TBSYS_LOG(ERROR, "rpc cs_scan ret=%d", ret);
  }
  else
  {
...
int ups_scan(const vector<string> &param)
{
  int ret = OB_SUCCESS;
  // scan table_id range_str columns_str version
  //
  if (param.size() < 4) 
  {
    fprintf(stderr, "scan table_id range_str columns_str version end_version\n");
    return OB_ERROR;
  }
  ...
  query_param.query_columns = param[2].c_str();
  query_param.version = strtoll(param[3].c_str(), NULL, 10);
  query_param.end_version = 0;
  if (param.size() > 4) query_param.end_version = strtoll(param[4].c_str(), NULL, 10);
  ...
  else if (OB_SUCCESS != (ret = GFactory::get_instance().get_rpc_stub().ups_scan(scan_param, scanner)))
  {
    TBSYS_LOG(ERROR, "rpc cs_scan ret=%d", ret);
  }
  else
  {
    ...
    int64_t fulfiiled_row_num = 0;
    scanner.set_is_req_fullfilled(is_fulfilled, fulfiiled_row_num);
    fprintf(stderr, "input range:[%s], ver:[%s]\n", 
        to_cstring(*scan_param.get_range()), to_cstring(scan_param.get_version_range()));
    fprintf(stderr, "new scanner, row count[%ld], is:[%d], fulfiiled_row_num:[%ld]\n", 
        scanner.get_row_num(), is_fulfilled, fulfiiled_row_num);
  }

  return ret;
}

int print_schema(const vector<string> &param)
{
  ObSchemaManagerV2 schema;
  int ret = OB_SUCCESS;
  int64_t version = 0;
  bool only_core_table = false;
  if (param.size() > 1) version = strtoll(param[0].c_str(), NULL, 10);
  if (param.size() > 2) only_core_table = strtoll(param[1].c_str(), NULL, 10);
  if (OB_SUCCESS != (ret = GFactory::get_instance().get_rpc_stub().fetch_schema(version, only_core_table, schema)))
  {
    fprintf(stderr, "fetch_schema return ret=%d", ret);
  }
  else
  {
    schema.print(stderr);
...
int sstable_scan(const vector<string> &param)
{
  int rc = OB_SUCCESS;
  int64_t network_timeout = 10 * 1000 * 1000; // 10s
  const char *help_msg = "sstable_scan table_id range_str columns_str version [index_table_id local_index_range_str]";
  if (param.size() < 4)
  {
    fprintf(stderr, "%s\n", help_msg);
    return OB_ERROR;
  ...
  QueryParam query_param;
  QueryParam local_index_param;
  int idx = 0;
  query_param.table_id = strtoll(param[idx++].c_str(), NULL, 10);
  query_param.scan_range = param[idx++].c_str();
  query_param.query_columns = param[idx++].c_str();
  query_param.version = strtoll(param[idx++].c_str(), NULL, 10);
  ...
  if (param.size() > 4)
  {
    ...
    local_index_param.table_id = strtoll(param[idx++].c_str(), NULL, 10);
    local_index_param.scan_range = param[idx++].c_str();
  }

  ObSSTableScanParam scan_param;
  ObRowDesc row_desc;

  rc = build_sstable_scan_param(query_param, param.size() <= 4 ? NULL : &local_index_param,
      scan_param, row_desc);
  if (OB_SUCCESS != rc)
  {
    fprintf(stderr, "build_sstable_scan_param() failed, rc %d\n", rc);
    return rc;
  }

  ObRemoteSSTableScan remote_sstable_scan;
  rc = remote_sstable_scan.set_param(scan_param, GFactory::get_instance().get_sql_rpc_stub(),
      GFactory::get_instance().get_chunk_server(), network_timeout);
  if (OB_SUCCESS != rc)
  {
    fprintf(stderr,"remote sstable scan set param failed, rc %d", rc);
    return rc;
  }

  rc = remote_sstable_scan.open();
  if (OB_SUCCESS != rc)
  {
    fprintf(stderr,"remote sstable scan open failed, rc %d", rc);
    return rc;
  }

  dump_phy_operator(remote_sstable_scan);

  rc = remote_sstable_scan.close();
  if (OB_SUCCESS != rc)
  {
    fprintf(stderr,"remote sstable scan close failed, rc %d", rc);
    return rc;
  }
...
int stop_build_index(const vector<string> &param)
{
  UNUSED(param);
  return GFactory::get_instance().get_rpc_stub().cs_stop_build_index();
}
...
int do_work(const int cmd, const vector<string> &param)
{
  ...
  switch (cmd)
  {
    ...
    case CMD_UPS_SCAN:
      ret = ups_scan(param);
      break;
    case CMD_SSTABLE_SCAN:
      ret = sstable_scan(param);
      break;
    ...
    case CMD_STOP_BUILD_INDEX:
      ret = stop_build_index(param);
      break;
      /*
    case CMD_GET:
      ret = get(param);
      break;
      */
    case CMD_PRINT_SCHEMA:
      ret = print_schema(param);
      break;
    default:
...
void usage()
{
  fprintf(stderr, "usage: ./cs_admin -s chunkserver_ip -p chunkserver_port -i \"command args\"\n"
          ...
          "sync_images delete_table load_sstables uninstall_disk install_disk\n\n"
  ...
  fprintf(stderr, "\tscan_root_table table_name table_id rowkey_split app_name "
                  ...
                  "\t     datetime_4 8 YYMMDDHHMMSS \n"
                  "\t     datetime_5 9 YYMMDD)\n");
  fprintf(stderr, "\tsstable_scan table_id range_str columns_str version [index_table_id local_index_range_str]\n");
  fprintf(stderr, "\tmigrate_tablet table range range_format dest_server keep_src\n");
  fprintf(stderr, "\tcreate_tablet table range range_format last_frozen_version\n");
  fprintf(stderr, "\tdelete_tablet table range range_format tablet_version is_force\n"
  ...
  fprintf(stderr, "\tstop_build_index\n");
  fprintf(stderr, "\tsync_images \n");
  fprintf(stderr, "\tinstall disk_no \n");
  fprintf(stderr, "\tuninstall disk_no \n");
...
int main(const int argc, char *argv[])
{
  ...
  while((ret = getopt(argc,argv,"s:p:i:f:ql:")) != -1)
  {
    switch(ret)
    {
      case 's':
