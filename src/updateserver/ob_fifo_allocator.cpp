...
namespace oceanbase
{
  ...
  namespace updateserver
  {
                            ...
                            const int64_t page_size)
    {
      ...
      else if (OB_SUCCESS != (ret = id_map_.init(total_limit / page_size)))
      {
    ...
    void FIFOAllocator::destroy()
    {
      ...
      if (0 != id_map_.size())
      {
        TBSYS_LOG(ERROR, "still memory have not been free");
      }
    ...
    FIFOAllocator::Page *FIFOAllocator::alloc_page_()
    {
      ...
      else
      {
        ...
        TBSYS_LOG(WARN, "allocated size=%ld reach limit=%ld", allocated_size_, total_limit_);
      }
      return ret;
    }
