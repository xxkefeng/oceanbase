                                          ...
                                          ObDataBuffer &buffer)
    {
      ...
      else
      {
        ...
        if (OB_NEW_GET_REQUEST == pkt.get_packet_code())
        {
          ...
          const CommonSchemaManager *schema_mgr = NULL;
          UpsSchemaMgr &ups_schema_mgr = UPS.get_table_mgr().get_schema_mgr();
          int64_t rowkey_cell_count = 0;
          UpsSchemaMgrGuard guard;
          if (NULL == (schema_mgr = ups_schema_mgr.get_schema_mgr(guard)))
          {
            ret = OB_SCHEMA_ERROR;
          ...
          else
          {
            ...
            {
              const ObTableSchema *tschema = schema_mgr->get_table_schema(get_param[0]->table_id_);
              const ObRowkeyInfo &rowkey_info = tschema->get_rowkey_info();
          ...
          if (OB_SUCCESS == ret)
          {
            if(OB_SUCCESS != (ret = ObNewScannerHelper::get_row_desc(get_param, true, rowkey_cell_count, row_desc)))
            {
                                          ...
                                          ObDataBuffer &buffer)
    {
      ...
      else
      {
        ...
        if (OB_NEW_SCAN_REQUEST == pkt.get_packet_code())
        {
          ...
          {
            const ObTableSchema *tschema = schema_mgr->get_table_schema(scan_param.get_table_id());
            const ObRowkeyInfo &rowkey_info = tschema->get_rowkey_info();
