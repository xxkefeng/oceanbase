...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        scan_flag_(INVALID_SCAN_FLAG)
    {
      ...
      //row_desc_(construct)
      //column_ids_(not init)
      //column_objs_(not init)
    }
    ...
    int ObCompactSSTableScanner::set_scan_param(const ObSSTableScanParam* scan_param, const ScanContext* scan_context)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else if (OB_SUCCESS != (ret = build_column_index(row_store_type, table_id, row_count_flag)))
        {
    ...
    int ObCompactSSTableScanner::get_next_row(const common::ObRow*& row_value)
    {
      int ret = OB_SUCCESS;
      ...
      FILL_TRACE_LOG("compact_sstable_scanner_get_next_row");
    ...
    int ObCompactSSTableScanner::convert_column_id(const uint64_t table_id)
    {
      ...
      {
        const ObSSTableSchemaColumnDef* def = NULL;
        if (NULL == (def = schema->get_table_schema(table_id, true, rowkey_column_cnt)))
        {
          TBSYS_LOG(WARN, "schema get table schema error: table_id=[%lu]", table_id);
          ret = OB_ERROR;
        }
        else
        {
          max_scan_column_offset_ = rowkey_column_cnt - 1;
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < scan_column_array_size; i ++)
      {
        ...
        else if (NULL != (def = schema->get_column_def(table_id, cur_column_id)))
        {
          ...
          {//row value
            if (OB_SUCCESS != (ret = scan_column_indexes_.add_column_id(ObSSTableScanColumnIndexes::Normal, def->offset_ + rowkey_column_cnt, cur_column_id)))
            {
              TBSYS_LOG(WARN, "scan column indexes add column id error: i=[%ld], ret=[%d], scan_column_indexes_=[%s], "
                  "offset=[%ld], cur_column_id=[%lu]", i, ret, to_cstring(scan_column_indexes_), def->offset_, cur_column_id);
              break;
            ...
            {
              if (def->offset_ + rowkey_column_cnt > max_scan_column_offset_)
              {
                max_scan_column_offset_ = def->offset_ + rowkey_column_cnt;
              }
            }
          }
        }
      ...
      if (OB_SUCCESS == ret)
      {
        if (DENSE_DENSE == row_store_type || DENSE_SPARSE == row_store_type)
        {
          for (int64_t i = 0; OB_SUCCESS == ret && i < column_id_array_size; i ++)
          {
            ...
            {
              TBSYS_LOG(WARN, "row desc add column desc error: i=[%ld], ret=[%d], table_id=[%lu], column_id=[%lu]",
                  i, ret, table_id, column_id_array_ptr[i]);
            }
          }
        }
    ...
    int ObCompactSSTableScanner::build_column_index(
        const ObCompactStoreType row_store_type,
        const uint64_t table_id,
        const RowCountFlag row_count_flag)
    {
      int ret = OB_SUCCESS;
      const uint64_t* const column_id_array_ptr = sstable_scan_param_->get_column_id();
      const int64_t column_id_array_size = sstable_scan_param_->get_column_id_size();
      ...
      if (OB_SUCCESS == ret)
      {
        if (DENSE_DENSE == row_store_type)
        {
          if (sstable_scan_param_->is_full_row_scan())
          {
            ...
            max_scan_column_offset_ = OB_MAX_COLUMN_NUMBER - 1;
          }
          else if (DENSE_DENSE_READER_NULL != row_count_flag
              && DENSE_DENSE_ROW_NULL != row_count_flag)
          {
            if (OB_SUCCESS != (ret = convert_column_id(table_id)))
            {
              TBSYS_LOG(WARN, "convert column id error: ret=[%d], table_id=[%lu]", ret, table_id);
            }
            else
            {
    ...
    int ObCompactSSTableScanner::deserialize_row(ObCompactCellIterator& row)
    {
      ...
      row_column_cnt_ = OB_MAX_COLUMN_NUMBER + 1;

      if (DENSE_DENSE == row_store_type)
      {
        if (OB_SUCCESS != (ret = row.get_one_row_dense_dense(column_objs_, max_scan_column_offset_ + 1, row_column_cnt_)))
        {
          TBSYS_LOG(WARN, "get one row of DENSE_DENSE error: ret=[%d]", ret);
        }
      }
      else if (DENSE_SPARSE == row_store_type)
      {
        if (OB_SUCCESS != (ret = row.get_one_row_dense_sparse(column_objs_, column_ids_, row_column_cnt_)))
        {
          TBSYS_LOG(WARN, "get one row of DENSE_SPARSE error: ret=[%d]", ret);
        }
      }
    ...
    int ObCompactSSTableScanner::load_block_index(const bool first_time)
    {
      int ret = OB_SUCCESS;
    ...
    int ObCompactSSTableScanner::dense_dense_store_row(const int64_t scan_column_cnt)
    {
      ...
      {
        for (int64_t i = 0; i < row_column_cnt_; i ++, index ++)
        {
          if (OB_SUCCESS != (ret = row_value_.raw_set_cell(index, column_objs_[i])))
          {
            TBSYS_LOG(WARN, "row value raw set cell error: i=[%ld], ret=[%d], index=[%ld], obj=[%s]",
                i, ret, index, to_cstring(column_objs_[i]));
            break;
          }
        }
      }
      else if (DENSE_DENSE_NORMAL_ROW_SCAN == scan_flag_)
      {
        ...
        {
          if (OB_SUCCESS != (ret = scan_column_indexes_.get_column(index, column)))
          {
              TBSYS_LOG(WARN, "scan column indexes get column: ret=[%d], index=[%ld]", ret, index);
              break;
          }
          else if (ObSSTableScanColumnIndexes::Rowkey == column.type_)
          {
            if (OB_SUCCESS != (ret = row_value_.raw_set_cell(index, column_objs_[column.index_])))
            {
              TBSYS_LOG(WARN, "row value set cell error:ret=[%d], index=[%ld], column.index_=[%ld], obj=[%s], row_value=[%s]",
                  ret, index, column.index_, to_cstring(column_objs_[column.index_]), to_cstring(row_value_));
              break;
            }
          }
    ...
    int ObCompactSSTableScanner::dense_sparse_store_row(const int64_t scan_column_cnt)
    {
      ...
      else
      {
        ...
        {
          column_ids_[i] = def_array[i].column_id_;
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < row_column_cnt_; i ++)
      {
