...
{ 
  namespace sstable 
  {
        ...
        int allocate_store()
        {
          ...
          if (NULL == column_info_)
          {
            if (NULL == column_info_ && NULL == (column_info_= reinterpret_cast<Column*>
                  (common::ob_malloc(sizeof(Column) * column_info_size_))))
            {
          ...
          if (NULL == index_map_)
          {
            if (NULL == (index_map_ = reinterpret_cast<int64_t*>
                  (common::ob_malloc(sizeof(int64_t) * common::OB_MAX_COLUMN_NUMBER))))
            {
              TBSYS_LOG(WARN, "failed to allocate memory for column index array");
              ret = common::OB_ALLOCATE_MEMORY_FAILED;
            }
    ...
    {
      public:
        ObSimpleColumnIndexes (const int16_t column_size = common::OB_MAX_COLUMN_NUMBER)
      ...
      private:
        int allocate_store()
        {
          ...
          if (NULL == column_info_)
          {
            if (NULL == (column_info_= reinterpret_cast<int16_t*>
                  (common::ob_malloc(sizeof(int16_t) * column_info_size_))))
            {
              TBSYS_LOG(WARN, "failed to allocate memory for column index array");
              ret = common::OB_ALLOCATE_MEMORY_FAILED;
            }
            ...
            {
              memset(column_info_, -1, sizeof(int16_t) * column_info_size_);
            }
          }
          return ret;
        }
      ...
      public:
        ...
        inline int add_column(int64_t offset)
        {
          ...
          {
            TBSYS_LOG(WARN, "add too many columm cnt=%d,size=%d", 
        ...
        inline int16_t find_by_offset(const int64_t offset) const
        {
          return column_info_[offset] ;
        }

        inline int16_t operator[](const int64_t offset) const
        {
          return find_by_offset(offset);
        }

        inline int16_t get_column_count() const 
        { 
          return column_cnt_;
        }

        inline int16_t get_not_null_column_count() const
        {
          return not_null_column_cnt_;
        }

        inline const int16_t* get_column_info() const
        {
          return column_info_;
        }
      ...
      private:
        uint64_t table_id_;
        int16_t column_cnt_;
        int16_t not_null_column_cnt_;
        int16_t column_info_size_;
        int16_t *column_info_;
    };
