...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      virtual int set_scalar_agg(const ObScalarAggregate &scalar_agg);
      virtual int set_group(const ObMergeGroupBy &group);
      virtual int set_group_columns_sort(const ObSort &sort);
      virtual int set_filter(const ObFilter &filter);
      virtual int add_filter(const ObSqlExpression & cond);
