 ...
 *
 */
#ifndef OCEANBASE_SQL_GET_SIMPLE_PARAM_H_
#define OCEANBASE_SQL_GET_SIMPLE_PARAM_H_
...
namespace oceanbase
{
  namespace sql 
  {
    class ObSqlGetSimpleParam : public ObSqlReadSimpleParam
    {
      static const int64_t DEFAULT_ROW_BUF_SIZE = 64 * 1024; // 16KB
      static const int64_t MAX_ROW_CAPACITY  = 10240;
    public:
      explicit ObSqlGetSimpleParam();
      virtual ~ObSqlGetSimpleParam ();
      void reset();
      void reset_rowkey_list();
      int destroy();
      ...
      inline const ObArray<common::ObRowkey>* get_rowkey_list() {return &rowkey_list_;}
      ...
      inline int64_t get_row_size() const { return rowkey_list_.count(); }
    ...
    private:
      DISALLOW_COPY_AND_ASSIGN(ObSqlGetSimpleParam);
      int32_t max_row_capacity_;
      ObArray<common::ObRowkey>  rowkey_list_;
      ObStringBuf buffer_pool_;
    };

    inline const common::ObRowkey* ObSqlGetSimpleParam::operator[](int64_t index) const 
    {
...
#endif /* end of include guard: OCEANBASE_SQL_GET_SIMPLE_PARAM_H_ */
