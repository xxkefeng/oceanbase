...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    int ObMergeServerService::reload_config(const char * config_file)
    {
      ...
      TBSYS_LOG(INFO, "reload config");
      int ret = OB_SUCCESS;
      if ((NULL == config_file) || (NULL == merge_server_ ) || (NULL == rpc_proxy_))
      {
        TBSYS_LOG(ERROR, "check merge server failed:config[%s], server[%p], rpc[%p]",
          config_file, merge_server_, rpc_proxy_);
        ret = OB_ERROR;
      }
      else
      {
      ...
      const int64_t timeout_us)
    {
      ...
      if (rc == OB_SUCCESS)
      {
        ...
        case OB_UPS_RELOAD_CONF:
          rc = ms_reload(receive_time, version, channel_id, req, in_buffer, out_buffer, timeout_us);
          break;
