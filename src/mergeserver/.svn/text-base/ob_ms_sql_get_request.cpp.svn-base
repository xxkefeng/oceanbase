...
int ObMsSqlGetRequest::set_request_param(ObSqlPlanParam &plan_param)
{
  int err = OB_SUCCESS;
  OB_ASSERT(plan_param.max_parallel_count_ > 0);
  OB_ASSERT(plan_param.initial_timeout_us_ > 0);
  max_parellel_count_ = plan_param.max_parallel_count_;
  max_get_rows_per_subreq_ = DEFAULT_MAX_GET_ROWS_PER_SUBREQ; //max_get_rows_per_subreq;
  reserve_get_param_count_ = DEFAULT_RESERVE_PARAM_COUNT; // reserve_get_param_count;
  // get request的get param
  get_param_ = &plan_param.get_request_get_param();
  phy_plan_ = &plan_param.inner_plan_;
  op_tablet_get_ = plan_param.get_op_tablet_get();

  if (NULL == op_tablet_get_)
  {
    TBSYS_LOG(WARN, "plan_param->get_op_sstable_get() is NULL");
    err = OB_NOT_INIT;
  }
  else if (!req_dist_map_inited_ && (OB_SUCCESS == err) && (0 != req_dist_map_.create(HASH_BUCKET_NUM)))
  {
  ...
  get_param_->set_data_version(plan_param.data_version_); // update every get request
  return err;
}
...
int ObMsSqlGetRequest::send_request(const int32_t sub_req_idx, const int64_t timeout_us)
{
  int err = OB_SUCCESS;
  // 引用
  /* inject cur_get_param to physical plan */
  OB_ASSERT(NULL != op_tablet_get_);
  ObSqlGetSimpleParam &cur_get_param = op_tablet_get_->get_get_param();
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != (err = get_rpc()->get(
            static_cast<int64_t>(timeout_us * get_timeout_percent() / 100),
            loc_list[svr_idx].server_.chunkserver_,
            *phy_plan_,
            *rpc_event)))
    {
      TBSYS_LOG(WARN,"fail to send request to server [request_id:%lu,err:%d]", get_request_id(),err);
