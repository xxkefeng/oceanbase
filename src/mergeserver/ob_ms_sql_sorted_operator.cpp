  ...
  bool &is_finish, ObStringBuf &rowkey_buffer)
{
  ...
  else if (sharding_result_count_ >= MAX_SHARDING_RESULT_COUNT)
  {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      sharding_result_arr_[sharding_result_count_].init(sharding_res,query_range,*scan_param_, stored_rowkey, fullfilled_item_num);
