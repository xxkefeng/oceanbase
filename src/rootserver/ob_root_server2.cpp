...
{
  UNUSED(did_write_log);
  int ret = OB_SUCCESS;
  config_.print();
  if (0 >= config_.tablet_replicas_num
           || OB_SAFE_COPY_COUNT < config_.tablet_replicas_num)
  {
    TBSYS_LOG(WARN, "invalid tablet_replicas_num=%s, max_replicas_num=%d, set replicas num to max_value",
        config_.tablet_replicas_num.str(), OB_SAFE_COPY_COUNT);
    // config_.tablet_replicas_num_ = OB_SAFE_COPY_COUNT;
    ret = OB_INVALID_ARGUMENT;
  }
  else if (MIN_BALANCE_TOLERANCE > config_.balance_tolerance_count)
  {
    TBSYS_LOG(ERROR, "invalid flag, balance_tolerence=%s",
              config_.balance_tolerance_count.str());
    ret = OB_INVALID_ARGUMENT;
  }
  else if (1 > config_.balance_worker_idle_time)
  {
    TBSYS_LOG(ERROR, "invalid flag, balance_worker_idle_time=%s",
  ...
  if (OB_SUCCESS == ret && config_.get_root_server(my_addr_))
  {
    ret = OB_INVALID_ARGUMENT;
  }
...
bool ObRootServer2::check_all_tablet_merged(void) const
{
  bool ret = false;
  // root table is valid
  int err = check_tablet_version(last_frozen_mem_version_, ret);
  if (err != OB_SUCCESS)
  {
    TBSYS_LOG(WARN, "check tablet merged failed:version[%ld], ret[%d]", last_frozen_mem_version_, err);
  }
  else if (true == ret)
  {
    TBSYS_LOG(TRACE, "check tablet merged succ:version[%ld]", last_frozen_mem_version_);
    tbsys::CRLockGuard guard(root_table_rwlock_);
    if (root_table_ != NULL)
    {
      ...
      {
        TBSYS_LOG(INFO, "check tablet merged succ:version[%ld], result[%d]", last_frozen_mem_version_, ret);
      }
    }
  }
  return ret;
