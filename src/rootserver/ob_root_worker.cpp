        ...
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
    {
      ...
      switch(msg.type)
      {
        case REFRESH_NEW_SCHEMA_TRIGGER:
          {
            ...
            {
              // TBSYS_LOG(INFO, "[TRIGGER][REFRESH_NEW_SCHEMA_TRIGGER]");
              int64_t count = 0;
              ret = root_server_.renew_user_schema(count);
              TBSYS_LOG(INFO, "[TRIGGER][renew_user_schema] done. ret=%d", ret);
            }
            break;
          }
        case REFRESH_NEW_CONFIG_TRIGGER:
          {
            // TBSYS_LOG(INFO, "[TRIGGER][REFRESH_NEW_CONFIG_TRIGGER]");
        ...
        case UPDATE_PRIVILEGE_TIMESTAMP_TRIGGER:
          {
            root_server_.set_privilege_version(tbsys::CTimeUtil::getTime());
            TBSYS_LOG(INFO, "[TRIGGER][UPDATE_PRIVILEGE_TIMESTAMP_TRIGGER]");
            break;
          }
        case SLAVE_BOOT_STRAP_TRIGGER:
          {
            if (role != common::ObiRole::MASTER)
            {
              ...
              // (2) update all server schema
              // TBSYS_LOG(INFO, "[TRIGGER][SLAVE_BOOT_STRAP_TRIGGER]");
              if (OB_SUCCESS != (ret = root_server_.slave_boot_strap()))
              {
                TBSYS_LOG(WARN, "fail to do slave boot strap. ret=%d", ret);
              }
              TBSYS_LOG(INFO, "[TRIGGER][SLAVE_BOOT_STRAP_TRIGGER] done. ret=%d", ret);
            }
            break;
          }
        default:
