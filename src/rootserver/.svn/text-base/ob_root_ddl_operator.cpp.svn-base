...
int ObRootDDLOperator::create_table(const TableSchema & table_schema)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (TableSchema::INDEX != table_schema.get_table_type()) // index table should not create empty tablet
    {
    ...
    else
    {
      ...
      if (OB_SUCCESS != (ret = root_server_->fetch_mem_version(mem_version)))
      {
        TBSYS_LOG(WARN, "fail to get mem_version. ret=%d", ret);
      }
      else
      {
        const_cast<TableSchema &>(table_schema).create_mem_version_ = mem_version;
      }
    }
  }
  if (OB_SUCCESS == ret)
...
int ObRootDDLOperator::modify_table_schema(const uint64_t table_id, TableSchema & table_schema)
{
  ...
  if (TableSchema::INDEX != table_schema.table_type_)
  {
...
int ObRootDDLOperator::modify_table_id(const common::TableSchema &table_schema, const int64_t new_table_id)
{
  int ret = OB_SUCCESS;
  if (!check_inner_stat())
  {
...
int ObRootDDLOperator::modify_index_table_status(
   const ObString& index_table_name, const int32_t state)
{
  int ret = OB_SUCCESS;
  ...
  {
    tbsys::CThreadGuard lock(&mutex_lock_);
    ret = schema_client_->modify_index_table_status(index_table_name, state);
    if (OB_SUCCESS != ret)
    {
      TBSYS_LOG(WARN, "fail to modify index table status, index_table_name=%s, state=%d", 
                to_cstring(index_table_name), state);
    }
  }
