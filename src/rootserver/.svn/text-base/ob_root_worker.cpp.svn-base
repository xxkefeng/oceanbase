  ...
  namespace rootserver
  {
    ...
    int ObRootWorker::handlePacket(ObPacket *packet)
    {
      ...
      switch(packet_code)
      {
        ...
        case OB_RS_INNER_MSG_CHECK_LOAD_PROCSS:
                ...
                if ((void*)WRITE_THREAD_FLAG == args)
                {
                  ...
                  switch(packet_code)
                  {
                    ...
                    case OB_SET_OBI_ROLE_TO_SLAVE:
                      ...
                      break;
                    case OB_CS_REPORT_SAMPLES:
                      return_code = rt_report_samples(version, *in_buf, req, channel_id, thread_buff);
                      break;
                    case OB_CS_SAMPLE_TABLE_DONE:
                      return_code = rt_cs_build_sample_done(version, *in_buf, req, channel_id, thread_buff);
                      break;
                    case OB_CS_BUILD_INDEX_DONE:
                      return_code = rt_cs_build_index_done(version, *in_buf, req, channel_id, thread_buff);
                      break;
                    default:
        ...
        bool use_inner_port /* = false*/)
    {
      ...
      {
        result_msg.result_code_ = root_server_.get_update_server_info(use_inner_port, found_server);
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        switch (table_id)
        {
          case OB_ALL_SERVER_STAT_TID:
            ...
            break;
          case OB_ROOT_TABLE_TID:
            rc.result_code_ = root_server_.dump_root_table(*new_scanner);
            break;
          default:
        ...
        if(OB_SUCCESS != rc.result_code_)
        {
          TBSYS_LOG(WARN, "open query service for table[%ld] fail:err[%d]", table_id, rc.result_code_);
        }
      }
        ...
        easy_request_t* req, const uint32_t channel_id, ObDataBuffer& out_buff)
    {
      ...
      if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_)
      {
        ...
        else if ((*get_param)[0]->table_id_ == OB_ROOT_TABLE_TID)
        {
          result_msg.result_code_ = root_server_.find_rootserver_key(*get_param, *scanner);
        }
        else
        {
        ...
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
    {
      ...
      {
        ObServer ups;
        err = root_server_.get_update_server_info(false, ups);
        if (OB_SUCCESS == err)
        {
          err = rt_rpc_stub_.get_last_frozen_version(ups,
        ...
        {
          TBSYS_LOG(WARN, "fail to get master ups[%d]", err);
        }
      }
      if (OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
      {
    ...
    int ObRootWorker::rt_report_samples(const int32_t version, common::ObDataBuffer& in_buff, 
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
    {
      ...
      else if (NULL == new_scanner)
      {
        TBSYS_LOG(ERROR, "failed to get thread local new scanner, new_scanner=%p", 
                  new_scanner);
        result_msg.result_code_ = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS != (result_msg.result_code_ = server.deserialize(
         in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position())))
      {
        TBSYS_LOG(ERROR, "server.deserialize error, ret=%d", ret);
      }
      else if (OB_SUCCESS != (result_msg.result_code_ = new_scanner->deserialize(
         in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position())))
      {
        TBSYS_LOG(ERROR, "new_scanner.deserialize error, ret=%d", result_msg.result_code_);
      }
      else if (OB_SUCCESS != (result_msg.result_code_ = root_server_.report_samples(server, *new_scanner)))
      {
        TBSYS_LOG(ERROR, "root server report samples error, ret=%d", result_msg.result_code_);
      }
      else if (OB_SUCCESS != (ret = result_msg.serialize(
         out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position())))
      {
        TBSYS_LOG(ERROR, "result_msg.serialize error, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = send_response(
         OB_CS_REPORT_SAMPLES_RESPONSE, MY_VERSION, out_buff, req, channel_id)))
      {
        TBSYS_LOG(ERROR, "send report samples response failed, ret=%d", ret);
      }

      return ret;
    }

    int ObRootWorker::rt_cs_build_sample_done(const int32_t version, common::ObDataBuffer& in_buff,
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
    {
      return build_index_done(version, in_buff, req, channel_id, out_buff, OB_CS_SAMPLE_TABLE_DONE_RESPONSE);;
    }

    int ObRootWorker::rt_cs_build_index_done(const int32_t version, common::ObDataBuffer& in_buff,
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
    {
      return build_index_done(version, in_buff, req, channel_id, out_buff, OB_CS_BUILD_INDEX_DONE_RESPONSE);
    }

    int ObRootWorker::build_index_done(const int32_t version, common::ObDataBuffer& in_buff,
        easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff, int response_code)
    {
      const int MY_VERSION = 1;
      common::ObResultCode result_msg;
      result_msg.result_code_ = OB_SUCCESS;
      ...
      else if (OB_SUCCESS != (result_msg.result_code_ = serialization::decode_bool(
         in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position(), &is_succ)))
      {
        TBSYS_LOG(ERROR, "is_succ flag deserialize error, ret=%d", result_msg.result_code_);
      }
      else if (OB_SUCCESS != (result_msg.result_code_ = server.deserialize(
         in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position())))
      {
        TBSYS_LOG(ERROR, "server.deserialize error, ret=%d", result_msg.result_code_);
      }
      else if (OB_SUCCESS != (result_msg.result_code_= serialization::decode_vi64(
         in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position(), 
         reinterpret_cast<int64_t*>(&index_table_id))))
      {
        TBSYS_LOG(ERROR, "index_table_id.deserialize error, ret=%d", result_msg.result_code_);
      }
      else if (OB_CS_SAMPLE_TABLE_DONE_RESPONSE == response_code
               && OB_SUCCESS != (result_msg.result_code_ = root_server_.cs_build_sample_done(
         is_succ, server, index_table_id)))
      {
        TBSYS_LOG(ERROR, "rootserver.cs_build_index_done error, response_code=%d, ret=%d", 
                  response_code, result_msg.result_code_);
      }
      else if (OB_CS_BUILD_INDEX_DONE_RESPONSE == response_code
               && OB_SUCCESS != (result_msg.result_code_ = root_server_.cs_build_index_done(
         is_succ, server, index_table_id)))
      {
        TBSYS_LOG(ERROR, "rootserver.cs_build_index_done error, response_code=%d, ret=%d", 
                  response_code, result_msg.result_code_);
         ...
         out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position())))
      {
        TBSYS_LOG(ERROR, "result_msg.serialize error, ret=%d", ret);
         ...
         response_code, MY_VERSION, out_buff, req, channel_id)))
      {
        TBSYS_LOG(ERROR, "send build index done response failed, ret=%d", ret);
      }
