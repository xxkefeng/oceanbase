...
namespace oceanbase
{
  namespace rootserver
  {
    ...
    const char* ROOT_1_TABLET_STATUS= "1_tablet_status";
    ...
    const char* ROOT_2_TABLET_STATUS= "2_tablet_status";
    ...
    const char* ROOT_3_TABLET_STATUS= "3_tablet_status";
...
bool ObRootServer2::init(const int64_t now, ObRootWorker* worker)
{
  ...
  else
  {
    ...
    else
    {
      ...
      schema_service_scan_helper_->set_scan_timeout(config_.network_timeout);
      schema_service_scan_helper_->set_mutate_timeout(config_.network_timeout);
...
void ObRootServer2::start_slave_rootserver()
{
  int ret = OB_SUCCESS;
...
int ObRootServer2::after_restart()
{
  ...
  else
  {
    ...
    //start building index if there are some index tables to build
    start_build_index_if_necessary();
  }
  return ret;
}

int ObRootServer2::notify_switch_schema(bool only_core_tables)
{
  int ret = OB_SUCCESS;
  ObSchemaManagerV2 *out_schema = OB_NEW(ObSchemaManagerV2, ObModIds::OB_RS_SCHEMA_MANAGER);
  if (OB_SUCCESS != (ret = get_schema(false, only_core_tables, *out_schema)))
  {
    ...
    else
    {
      ...
      else
      {
        while (OB_SUCCESS == ret
               && OB_SUCCESS == (ret = tables_id_name.get_next(&tid_name)))
        {
        ...
        if (OB_SUCCESS == ret)
        {
          if (!only_core_tables)
          {
            ...
            {
              TBSYS_LOG(INFO, "get schema succ. table count=%d", table_count);
            }
          }
          else
          {
...
int ObRootServer2::dump_root_table(ObNewScanner &scanner) const
{
  tbsys::CRLockGuard guard(root_table_rwlock_);
  TBSYS_LOG(INFO, "dump root table to scanner");
  if (root_table_ != NULL)
  {
    root_table_->dump(scanner, get_server_manager());
  }
  return OB_SUCCESS;
}
...
int ObRootServer2::create_table_tablets(const uint64_t table_id, const ObTabletInfoList & list, bool cs_create)
{
  int err = OB_SUCCESS;
  int64_t version = -1;
  ...
  else if (OB_SUCCESS != (err = create_tablet_with_range(version, list, cs_create)))
  {
...
int ObRootServer2::check_tablets_legality(const common::ObTabletInfoList &tablets)
{
  ...
  if (OB_SUCCESS == err)
  {
    for (int64_t i = 1; OB_SUCCESS == err && i < tablets.get_tablet_size(); i++)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "range not legal, should be left open right closed. range=%s", 
                  to_cstring(p_tablet_info->range_));
      }
      else if (p_tablet_info->range_.start_key_ == p_tablet_info->range_.end_key_)
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "range not legal, start_key = end_key; range=%s", 
                  to_cstring(p_tablet_info->range_));
      }
...
int ObRootServer2::create_tablet_with_range(const int64_t frozen_version, const ObTabletInfoList& tablets, bool cs_create)
{
  int ret = OB_SUCCESS;
      ...
      else
      {
        ...
        if (OB_SUCCESS != (ret =
                           create_empty_tablet_with_range(frozen_version, root_table_for_split,
                                                          *p_table_info, create_count[i], server_index[i], cs_create)))
        {
...
int ObRootServer2::create_empty_tablet_with_range(const int64_t frozen_version,
    ...
    int32_t& created_count, int* t_server_index, bool cs_create)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret)
  {
      ...
      if (server_index[i] != OB_INVALID_INDEX)
      {
        ...
        if (server_status != NULL)
        {
          ...
          if (cs_create)
          {
            ...
            err = worker_->get_rpc_stub().create_tablet(server, tablet.range_, frozen_version,
    ...
    if (created_count > 0)
    {
      ...
      ret = root_table->create_table(tablet, sia, frozen_version, cs_create);
      if (OB_SUCCESS != ret)
      {
...
int ObRootServer2::check_tablet_version(const int64_t tablet_version, bool &is_merged) const
{
  ...
  if (NULL != root_table_)
  {
    int64_t safe_count = config_.tablet_replicas_num;
    if ((chunk_server_count > 0) && (chunk_server_count < safe_count))
    {
      TBSYS_LOG(TRACE, "check chunkserver count less than replica num:server[%d], replica[%ld]",
          chunk_server_count, safe_count);
      safe_count = chunk_server_count;
    }
    ...
    {
      err = root_table_->check_tablet_version_merged(tablet_version, safe_count, is_merged);
    }
  }
  else
  {
...
void ObRootServer2::commit_task(const ObTaskType type, const ObRole role, const ObServer & server,
    int32_t inner_port, const char* server_version)
{
 ...
 * 迁移完成操作
 */
int ObRootServer2::migrate_over(const ObNewRange& range, const common::ObServer& src_server, const common::ObServer& dest_server, const bool keep_src, const int64_t tablet_version)
{
  int ret = OB_SUCCESS;
  TBSYS_LOG(INFO, "migrate_over received, src_cs=%s dest_cs=%s keep_src=%d "
      "range=%s", to_cstring(src_server),
      to_cstring(dest_server), keep_src, to_cstring(range));
                                 ...
                                 ObRootTable2::const_iterator last, ObScanner& scanner, const int32_t max_row_count, const int32_t max_key_len) const
{
  ...
  static ObString s_root_1_tablet_status(static_cast<int32_t>(strlen(ROOT_1_TABLET_STATUS)), static_cast<int32_t>(strlen(ROOT_1_TABLET_STATUS)), (char*)ROOT_1_TABLET_STATUS);
  ...
  static ObString s_root_2_tablet_status(static_cast<int32_t>(strlen(ROOT_2_TABLET_STATUS)), static_cast<int32_t>(strlen(ROOT_2_TABLET_STATUS)), (char*)ROOT_2_TABLET_STATUS);
  ...
  static ObString s_root_3_tablet_status(static_cast<int32_t>(strlen(ROOT_3_TABLET_STATUS)), static_cast<int32_t>(strlen(ROOT_3_TABLET_STATUS)), (char*)ROOT_3_TABLET_STATUS);
  ...
  for (ObRootTable2::const_iterator it = first; it <= last; it++)
  {
        ...
        (server_status = server_manager_.get_server_status(it->server_info_indexes_[0])) != NULL)
    {
      ...
      out_cell.column_name_ = s_root_1_tablet_status;
      out_cell.value_.set_int(it->tablet_status_[0]);
      if (OB_SUCCESS != (ret = scanner.add_cell(out_cell)))
      {
        break;
      }
        ...
        (server_status = server_manager_.get_server_status(it->server_info_indexes_[1])) != NULL)
    {
      ...
      out_cell.column_name_ = s_root_2_tablet_status;
      out_cell.value_.set_int(it->tablet_status_[1]);
      if (OB_SUCCESS != (ret = scanner.add_cell(out_cell)))
      {
        break;
      }
        ...
        (server_status = server_manager_.get_server_status(it->server_info_indexes_[2])) != NULL)
    {
      ...
      out_cell.column_name_ = s_root_3_tablet_status;
      out_cell.value_.set_int(it->tablet_status_[2]);
      if (OB_SUCCESS != (ret = scanner.add_cell(out_cell)))
      {
        break;
      }
...
int ObRootServer2::find_rootserver_key(const common::ObGetParam& get_param, common::ObScanner& scanner)
{
  int ret = OB_SUCCESS;
  const ObCellInfo* cell = NULL;

  TBSYS_LOG(DEBUG, "find rootserver key");
  if (NULL == (cell = get_param[0]))
  {
    TBSYS_LOG(WARN, "invalid get_param, cell_size=%ld", get_param.get_cell_size());
    ret = OB_INVALID_ARGUMENT;
  ...
  else
  {
    ...
    config_.get_root_server(rs_addr);
    monitor_table.init(rs_addr);
...
int ObRootServer2::find_root_table_range(const common::ObScanParam& scan_param, ObScanner& scanner)
{
  ...
  //support find tablet ranage by table_id or table_name, table name first
  if (NULL != table_name.ptr() && table_name.length() > 0
      && OB_SUCCESS != (ret = get_table_info(table_name, table_id, max_key_len)))
  {
    TBSYS_LOG(WARN, "failed to get table info from schema, ret=%d, table_name=%s",
              ret, to_cstring(table_name));
  }
  else if (0 == table_id || OB_INVALID_ID == table_id)
  {
    TBSYS_LOG(WARN,"table name are invaild:%s,%d, table_id=%lu",
              table_name.ptr(),table_name.length(), table_id);
    ret = OB_INVALID_ARGUMENT;
  }
...
int ObRootServer2::waiting_job_done(const common::ObServer& server, const int64_t frozen_mem_version)
{
  ...
  if (it != server_manager_.end())
  {
    ...
    if (is_bypass_process() && !operation_helper_.is_build_index())
    {
...
int ObRootServer2::got_reported(const ObTabletReportInfoList& tablets, const int server_index, const int64_t frozen_mem_version)
{
  ...
  if (ObBootState::OB_BOOT_OK != boot_state_.get_boot_state())
  {
    TBSYS_LOG(INFO, "rs has not boot strap, refuse report.");
  }
  else
  {
    for (int64_t i = 0; i < tablets.get_tablet_size(); i ++)
    {
      add_tablet.add_tablet(tablets.tablets_[i]);
    }
    got_reported_for_query_table(add_tablet, server_index, frozen_mem_version);
    ...
    const int32_t server_index, const int64_t frozen_mem_version)
{
  ...
  {
    if (state_.get_bypass_flag() && !operation_helper_.is_build_index())
    {
      ret = operation_helper_.report_tablets(tablets, server_index, frozen_mem_version);
       if (OB_SUCCESS != ret)
       {
         TBSYS_LOG(WARN, "fail to do bypass report tablet. ret=%d", ret);
       }
    }
    else
    {
  ...
  else
  {
    for (ObRootTable2::const_iterator it = first; it <= last; it++)
    {
      ...
      if (found_it_index != OB_INVALID_INDEX)
      {
        ...
        //CS only report available tablet
        atomic_exchange((uint32_t*) &(it->tablet_status_[found_it_index]), common::TABLET_AVAILABLE);
...
int ObRootServer2::select_cs(const int64_t select_num, ObArray<std::pair<ObServer, int32_t> > &chunkservers)
{
  int ret = OB_SUCCESS;
  ...
  else
  {
    ...
    do
    {
      ...
      for (; it != server_manager_.end() && results_num < select_num; ++it)
      {
        ++visit_count;
        ++select_pos;
        if (it->status_ != ObServerStatus::STATUS_DEAD)
...
int ObRootServer2::delete_dropped_tables(int64_t & table_count)
{
  ...
  {
    tbsys::CRLockGuard guard(root_table_rwlock_);
    ret = root_table_->get_deleted_table(*schema_manager_for_cache_, table_id);
    if ((OB_SUCCESS == ret) && (table_id != OB_INVALID_ID))
    {
      if (table_id < OB_ALL_SERVER_STAT_TID)
      {
        TBSYS_LOG(ERROR, "delete table id is less than OB_ALL_SERVER_STAT_TID:table_id[%lu]", table_id);
      }
      else
      {
        ret = deleted_tables.push_back(table_id);
        if (ret != OB_SUCCESS)
        {
          TBSYS_LOG(WARN, "push deleted table failed:table_id[%lu], ret[%d]", table_id, ret);
        }
        else
        {
          TBSYS_LOG(INFO, "find a table not in schema:table_id[%lu]", table_id);
        }
      }
    }
...
int ObRootServer2::create_table(bool if_not_exists, const common::TableSchema &tschema)
{
  ...
  if (!tschema.is_valid())
  {
    TBSYS_LOG(WARN, "table schema is invalid:table_name[%s]", tschema.table_name_);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    // fire an event to tell all clusters
    {
      err = ObRootTriggerUtil::create_table(root_trigger_);
      if (err != OB_SUCCESS)
      {
...
int ObRootServer2::drop_tables(bool if_exists, const common::ObStrings &tables)
{
  ...
  else if (true == is_all_merged)
  {
    ...
    // fire an event to tell all clusters
    {
      int err = ObRootTriggerUtil::drop_tables(root_trigger_);
      if (err != OB_SUCCESS)
      {
...
int ObRootServer2::get_update_server_info(bool use_inner_port, ObServer &server) const
{
  int ret = OB_SUCCESS;
  ObUps ups_master;
  if (NULL != ups_manager_)
  {
    ret = ups_manager_->get_ups_master(ups_master);
    if (OB_SUCCESS == ret)
    {
    ...
    {
      TBSYS_LOG(WARN, "no ups master[%d]", ret);
    }
  }
  return ret;
}
...
bool ObRootServer2::check_all_tablet_merged(void) const
{
  bool ret = false;
  int64_t cs_heartbeat_frozen_version = get_frozen_version_for_cs_heartbeat();
  // root table is valid
  int err = check_tablet_version(cs_heartbeat_frozen_version, ret);
  if (err != OB_SUCCESS)
  {
    TBSYS_LOG(WARN, "check tablet merged failed:version[%ld], ret[%d]", cs_heartbeat_frozen_version, err);
  }
  else if (true == ret)
  {
    TBSYS_LOG(TRACE, "check tablet merged succ:version[%ld]", cs_heartbeat_frozen_version);
    tbsys::CRLockGuard guard(root_table_rwlock_);
    if (root_table_ != NULL)
    {
      ...
      {
        TBSYS_LOG(INFO, "check tablet merged succ:version[%ld], result[%d]", cs_heartbeat_frozen_version, ret);
      }
    }
  }
  return ret;
...
int ObRootServer2::report_frozen_memtable(const int64_t frozen_version, const int64_t last_frozen_time, bool did_replay)
{
  ...
  {
    if (!did_replay && is_master() && (!check_all_tablet_merged()))
    {
...
int ObRootServer2::get_last_frozen_version_from_ups(const bool did_replay)
{
  ...
  int ret = OB_SUCCESS;
  ObServer ups;
  ret = get_update_server_info(false, ups);
  if (0 == ups.get_ipv4())
  {
  ...
  if (config_.enable_new_root_table)
  {
    if (is_master())
    {
      ...
      else
      {
        ...
        {
          ObRootMsProvider ms_provider(server_manager_);
          ObRootUpsProvider ups_provider(ups);
          ObScanHelperImpl scan_helper;
          scan_helper.set_ms_provider(&ms_provider);
          scan_helper.set_rpc_stub(&worker_->get_rpc_stub());
          scan_helper.set_ups_provider(&ups_provider);
        ...
        {
          TBSYS_LOG(WARN, "cannot get master ups[%d]", ret);
        }
      }
    }
  } // end if enable
  ...
  if (this->is_master())
  {
    ...
    for (; OB_SUCCESS == ret && it != this->server_manager_.end(); ++it)
    {
      if (it->status_ != ObServerStatus::STATUS_DEAD && it->port_cs_ != 0)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          TBSYS_LOG(INFO, "sync schema to cs %s, version[%ld]", to_cstring(tmp_server), schema.get_version());
        }
      ...
      if (OB_SUCCESS == ret && it->ms_status_ != ObServerStatus::STATUS_DEAD && it->port_ms_ != 0)
      {
        ...
        {
          // do nothing
          TBSYS_LOG(INFO, "sync schema to ms %s, version[%ld]", to_cstring(tmp_server), schema.get_version());
        }
        else if (OB_OLD_SCHEMA_VERSION == ret)
        {
          ret = OB_SUCCESS;
...
int ObRootServer2::prepare_bypass_process(common::ObBypassTaskInfo &table_name_id)
{
  int ret = OB_SUCCESS;
  tbsys::CWLockGuard guard(schema_manager_rwlock_);
  uint64_t max_table_id = schema_manager_for_cache_->get_max_table_id();
  ...
  {
    ddl_tool_.update_max_table_id(max_table_id);
    int64_t count = 0;
    if (OB_SUCCESS != (ret = refresh_new_schema(count)))
    {
    ...
    else
    {
      ...
      else
      {
        ...
        schema_manager_for_cache_->write_to_file(SCHEMA_FILE_NAME);
      }
    }
  }
  return ret;
...
int ObRootServer2::start_bypass_process(ObBypassTaskInfo &table_name_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_EAGAIN;
      TBSYS_LOG(WARN, "already have some bypass task doing. retry later.");
    }
    else
    {
      TBSYS_LOG(INFO, "bypass process start.");
      state_.set_bypass_flag(true);
    }
  }
  if (OB_SUCCESS == ret)
  {
    tbsys::CWLockGuard guard(schema_manager_rwlock_);
    ret = operation_helper_.start_operation(schema_manager_for_cache_, table_name_id, cs_heartbeat_frozen_version);
    if (OB_SUCCESS != ret)
    {
      state_.set_bypass_flag(false);
      TBSYS_LOG(WARN, "fail to start bypass process. ,set bypass flag false.ret=%d", ret);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      bypass_process_frozen_men_version_ = cs_heartbeat_frozen_version;
      TBSYS_LOG(INFO, "bypass process start. bypass process frozen_mem_version=%ld, operation_type=%d",
                bypass_process_frozen_men_version_, table_name_id.get_operation_type());
...
int ObRootServer2::check_bypass_process()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else if (OB_SUCCESS == ret)
    {
      ...
      log_worker_->set_frozen_version_for_brodcast(bypass_process_frozen_men_version_);
      if (INDEX_TABLE == type
          && OB_SUCCESS != (ret = start_build_index_if_necessary()))
      {
        TBSYS_LOG(WARN, "failed to start next build index process, ret=%d", ret);
      }
    }
    else
    {
...
int ObRootServer2::switch_bypass_schema(common::ObSchemaManagerV2 *schema_manager, common::ObArray<uint64_t> &delete_tables)
{
  ...
  if (OB_SUCCESS == ret)
  {
    for(int64_t i = 0; i < delete_tables.count(); i++)
    {
      ...
      {
        if (strncmp(new_table_schema.at(j).table_name_, table_name, OB_MAX_TABLE_NAME_LENGTH))
        {
          find = true;
    ...
    const uint64_t table_id, const bool is_succ)
{
  ...
  {
    int32_t cs_index = get_server_index(cs);
    operation_helper_.delete_tables_done(cs_index, table_id, is_succ);
  }
  else
  {
    ret = OB_ERROR;
...
int ObRootServer2::start_build_index_if_necessary()
{
  int ret = OB_SUCCESS;
  const ObTableSchema* table_schema = NULL;
  ObBuildIndexInfo build_index_info;

  if (!is_bypass_process() && is_master()
      && check_all_tablet_merged())
  {
    {
      tbsys::CRLockGuard guard(schema_manager_rwlock_);
      if (NULL != schema_manager_for_cache_)
      {
        for (table_schema = schema_manager_for_cache_->table_begin();
             table_schema != schema_manager_for_cache_->table_end();
             table_schema++)
        {
          if (need_build_index(*table_schema))
          {
            if (OB_SUCCESS != (ret = fill_build_index_info(build_index_info, *table_schema)))
            {
              TBSYS_LOG(WARN, "failed to fill build index task info, ret=%d", ret);
              table_schema = NULL;
            }
            break;
          }
      ...
      if (table_schema == schema_manager_for_cache_->table_end())
      {
        table_schema = NULL;
      }
    }
    if (NULL != table_schema
        && OB_SUCCESS != (ret = start_index_process(build_index_info)))
    {
      TBSYS_LOG(WARN, "failed to start sample process, ret=%d", ret);
    }
  }
...
bool ObRootServer2::need_build_index(const ObTableSchema& table_schema) const
{
  return (ObTableSchema::INDEX == table_schema.get_table_type()
          && ObTableSchema::UNAVAILABLE == table_schema.get_index_status()
          && table_schema.get_create_mem_version() < get_frozen_version_for_cs_heartbeat());
}

int ObRootServer2::fill_build_index_info(
   ObBuildIndexInfo& build_index_info, const ObTableSchema& index_table_schema)
{
  int ret = OB_SUCCESS;
  ...
  build_index_info.data_table_id_ = index_table_schema.get_data_table_id();
  build_index_info.index_table_id_ = index_table_schema.get_table_id();
  build_index_info.sample_count_ = 1;
  ...
  else if (root_table_->table_is_exist(build_index_info.index_table_id_))
  {
    if (!root_table_->check_table_all_tablets_available(build_index_info.index_table_id_))
    {
      //continue build index, needn't build sample again
      build_index_info.sample_count_ = 0;
    }
    else
    {
      TBSYS_LOG(WARN, "index table schema status is unavailable, but root table is good, "
                      "index_table_id=%lu",
                build_index_info.index_table_id_);
      ret = OB_ERROR;
    }
  }
  else if (OB_SUCCESS != (ret = root_table_->get_table_tablet_info(
           index_table_schema.get_data_table_id(), tablet_count, row_count, data_size)))
  {
    TBSYS_LOG(WARN, "failed to get table tablet info from root table, table_id=%lu, ret=%d",
              index_table_schema.get_data_table_id(), ret);
  }
  else if (0 == row_count)
  {
    build_index_info.sample_count_ = 1;
    TBSYS_LOG(INFO, "data table is empty, assign sample count to 1");
  }
  else
  {
    const ObColumnSchemaV2* columns = NULL;
    int32_t column_count = 0;
    int64_t estimated_row_size = 0;
    int64_t split_sstable_count = 0;
    int64_t max_sstable_size = index_table_schema.get_max_sstable_size();

    if (NULL ==(columns = schema_manager_for_cache_->get_table_schema(
        index_table_schema.get_table_id(), column_count)) || column_count <= 0)
    {
      TBSYS_LOG(WARN, "failed to get tablet schema from schema manager, "
                "index_table_id_=%lu, table_column_count=%d",
                index_table_schema.get_table_id(), column_count);
      ret = OB_ERROR;
    }
    else
    {
      ...
      {
        estimated_row_size += columns[i].get_size();
      }
    }

    if (OB_SUCCESS == ret)
    {
      if (estimated_row_size <= 0 || max_sstable_size <= 0)
      {
        TBSYS_LOG(WARN, "index table estimated row size is invalid, estimated_row_size=%ld, "
                  "max_sstable_size=%ld, index_table_id=%lu",
                  estimated_row_size, max_sstable_size, index_table_schema.get_table_id());
        ret = OB_ERROR;
      }
      else
      {
        /**
         * the index table only store rowkey column, so we only estimate the 
         * rowkey size, and assume that the compress ratio is 50%, then we can 
         * known how many sstable to split for this index table 
         * 
         * TODO: find a better algorithm to sstable split count 
         */
        split_sstable_count = (estimated_row_size * row_count / 2) / max_sstable_size;
        build_index_info.sample_count_ = split_sstable_count > 0 ? split_sstable_count : 1;
      }
    }
  }
...
int ObRootServer2::start_index_process(const ObBuildIndexInfo& build_index_info)
{
  int ret = OB_SUCCESS;

  ObBypassTaskInfo build_sample_task;
  build_sample_task.set_operation_type(INDEX_TABLE);
  build_sample_task.set_build_info(build_index_info);
  if (OB_SUCCESS != (ret = start_bypass_process(build_sample_task)))
  {
    TBSYS_LOG(WARN, "fail to start sample, ret=%d", ret);
  }

  return ret;
}

int ObRootServer2::report_samples(const ObServer& server, const ObNewScanner& sample_scanner)
{
  int ret = OB_SUCCESS;
  int server_index = get_server_index(server);
  ...
  else if (!is_bypass_process())
  {
    TBSYS_LOG(WARN, "rootserver not in build index process.");
    ret = OB_ERROR;
  }
  else if (is_master())
  {
    TBSYS_LOG(INFO, "[NOTICE] report samples, server=%d, ip=%s, row_count=%ld",
              server_index, to_cstring(server), sample_scanner.get_row_num());
    ret = operation_helper_.cs_report_samples(server_index, sample_scanner);
  }

  return ret;
}

int ObRootServer2::cs_build_sample_done(
   const bool is_succ, const ObServer& server, const int64_t index_table_id)
{
  int ret = OB_SUCCESS;
  int server_index = get_server_index(server);
  ...
  {
    ret = operation_helper_.cs_build_sample_done(is_succ, server_index, index_table_id);
  }

  return ret;
}

int ObRootServer2::cs_build_index_done(
   const bool is_succ, const ObServer& server, const int64_t index_table_id)
{
  int ret = OB_SUCCESS;
  int server_index = get_server_index(server);
  ...
  {
    ret = operation_helper_.cs_build_index_done(is_succ, server_index, index_table_id);
  }

  return ret;
}

int ObRootServer2::update_index_table_schema_state(const uint64_t index_table_id, int state)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_INVALID_ID == index_table_id || state > ObTableSchema::INDEX_ERROR)
  {
    TBSYS_LOG(WARN, "invalid param, index_table_id=%lu, state=%d",
              index_table_id, state);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if (NULL == schema_manager_for_cache_)
    {
      TBSYS_LOG(WARN, "schema manager is NULL");
      ret = OB_INNER_STAT_ERROR;
    }
    else if (NULL == (index_table_schema = schema_manager_for_cache_->get_table_schema(index_table_id)))
    {
      TBSYS_LOG(WARN, "failed to get index table schema, index_table_id=%lu", 
                index_table_id);
      ret = OB_ERROR;
    }
    else
    {
      strncpy(table_name_str, index_table_schema->get_table_name(), OB_MAX_TABLE_NAME_LENGTH);
      index_table_name = ObString::make_string(table_name_str);
    }
  }

  if (OB_SUCCESS == ret
      && OB_SUCCESS != (ret = ddl_tool_.modify_index_table_status(index_table_name, state)))
  {
    TBSYS_LOG(WARN, "failed to modify index table status, index_table_id=%lu, state=%d",
              index_table_id, state);
  }
  else if (OB_SUCCESS != (ret = renew_user_schema(count)))
  {
    TBSYS_LOG(WARN, "refresh new schema manager after modify index "
                    "table status failed. ret=%d", ret);
  }
  else
  {
    TBSYS_LOG(INFO, "update index table schema state success, index_table_id=%lu, state=%d",
              index_table_id, state);
  }
...
int ObRootServer2::build_index_root_table()
{
  int ret = OB_SUCCESS;
  ObIndexOperation& index_builder = operation_helper_.get_index_builder();
  const ObBuildIndexInfo& build_index_info = index_builder.get_build_index_info();
  bool table_not_exist_in_rt = false;
  bool is_exist_in_cs = true;

  {
    ...
    else if (OB_SUCCESS != (ret = index_builder.build_sample_result(schema_manager_for_cache_)))
    {
      TBSYS_LOG(WARN, "index builder fails to build sample result, ret=%d", ret);
    }
  }
  
  if (OB_SUCCESS == ret)
  {
    TBSYS_LOG(INFO, "build index root table start: index_table_id=%lu", 
              build_index_info.index_table_id_);
    tbsys::CThreadGuard mutex_gard(&root_table_build_mutex_);
    if (NULL == root_table_)
    {
    ...
    else
    {
      {
        tbsys::CRLockGuard guard(root_table_rwlock_);
        if(!root_table_->table_is_exist(build_index_info.index_table_id_))
        {
          TBSYS_LOG(INFO, "table not exist, index_table_id=%lu", 
                    build_index_info.index_table_id_);
          table_not_exist_in_rt = true;
        }
      }
      ...
      if (table_not_exist_in_rt)
      {
        ...
        {
          //check table not exist in chunkserver
          if (OB_SUCCESS != (ret = table_exist_in_cs(build_index_info.index_table_id_, is_exist_in_cs)))
          {
            TBSYS_LOG(WARN, "fail to check table. index_table_id=%lu, ret=%d", 
      ...
      if (!is_exist_in_cs)
      {
        ...
        while (OB_SUCCESS == ret && !is_iter_end)
        {
          ret = index_builder.get_next_sample_result(tablets);
          if (OB_ITER_END == ret)
          {
            is_iter_end = true;
            ret = OB_SUCCESS;
          ...
          if (OB_SUCCESS == ret && tablets.get_tablet_size() > 0
              && OB_SUCCESS != (ret = create_table_tablets(build_index_info.index_table_id_, tablets, false)))
          {
            TBSYS_LOG(WARN, "fail to create table[%lu] tablets, tablet_count=%ld, "
                            "is_iter_end=%d, ret=%d", 
                      build_index_info.index_table_id_, tablets.get_tablet_size(), is_iter_end, ret);
          }
        }
      }
    }
