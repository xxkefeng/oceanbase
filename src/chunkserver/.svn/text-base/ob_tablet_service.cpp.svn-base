...
int ObTabletService::open(const sql::ObSqlReadParam &sql_read_param)
{
  ...
  FILL_TRACE_LOG("tablet_service_open_start");
  ...
  FILL_TRACE_LOG("prepare_work_done");
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      FILL_TRACE_LOG("create_plan_done");
      ...
      FILL_TRACE_LOG("tablet_read_open_done");
    }
  }
...
int ObTabletService::open(const sstable::ObSSTableScanParam &scan_param,
    const int64_t timeout_us)
{
  ...
  tablet_sstable_scan_.set_param(scan_param, scan_context);

  tablet_sstable_scan_.set_is_read_consistency(scan_param.get_is_read_consistency());

  // schema manager not used in create_plan
  if (OB_SUCCESS != (rc = tablet_sstable_scan_.create_plan(*(const ObSchemaManagerV2 *)NULL)))
  {
    TBSYS_LOG(ERROR, "tablet sstable scan create plan failed, rc %d", rc);
  }
  else if (OB_SUCCESS != (rc = tablet_sstable_scan_.open()))
  {
    TBSYS_LOG(ERROR, "open tablet sstable scan operator failed, rc %d", rc);
  }
  else
  {
    tablet_read_ = &tablet_sstable_scan_;
  }
  ...
  while(OB_SUCCESS == ret)
  {
    if(NULL == cur_row_)
    {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        if(OB_SUCCESS == ret)
        {
          if(OB_SUCCESS != (ret = tablet_read_->get_cur_rowkey(cur_rowkey_)))
          {
...
int ObTabletService::close()
{
  ...
  if (OB_SUCCESS != (err = tablet_sstable_scan_.close()))
  {
    ret = err;
    TBSYS_LOG(WARN, "close tablet sstable scan fail:err[%d]", err);
  }
  return ret;
}
