...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObBypassSSTableLoader::finish_load()
    {
      ...
      if(OB_SUCCESS != (ret = tablet_manager_->get_serving_tablet_image().flush_log(OB_LOG_CS_BYPASS_LOAD_TABLET, OB_SUCCESS == ret)))
      {
      ...
      for (; it != tablet_array_.end(); ++it)
      {
        ...
        if (NULL != *it)
        {
          ...
          if (OB_SUCCESS == tablet_image.include_sstable(sstable_id))
          {
            ...
            ret = tablet_image.remove_tablet(tablet->get_range(), tablet->get_data_version(), disk_no);
            if (OB_SUCCESS != ret)
            {
          ...
          tablet_image.release_tablet(*it);
        }
      }
    ...
    int ObBypassSSTableLoader::recycle_unload_tablets()
    {
      int ret = OB_SUCCESS;
      ...
      for (; it != tablet_array_.end(); ++it)
      {
        if (NULL != *it)
        {
          if (OB_SUCCESS != (ret = tablet_image.add_tablet(
            *it, true, tablet_image.get_last_not_merged_version() == 0)))
          {
