  ...
  namespace chunkserver
  {
    ...
    int ObTabletMergerV2::prepare_merge(ObTablet *tablet, int64_t frozen_version)
    {
            ...
            chunk_merge_.current_schema_.get_table_schema(tablet->get_range().table_id_)) )
      {
        ...
        if(OB_SUCCESS != (ret = manager_.get_serving_tablet_image().set_tablet_merged(tablet)))
        {
          TBSYS_LOG(WARN, "set tablet merged failed, ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = manager_.get_serving_tablet_image().flush_log(OB_LOG_CS_MERGE_TABLET)))
        {
          TBSYS_LOG(WARN, "flush log failed, ret=%d", ret);
        }
        else
        {
