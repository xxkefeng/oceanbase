 ...
 *
 * Time-stamp: <2013-05-21 10:36:30 fufeng.syd>
 * Version: $Id$
  ...
  else
  {
    ...
    else
    {
      ...
      do
      {
        ...
        if (OB_SUCCESS !=
            (ret = rc.deserialize(msgbuf.get_data(), msgbuf.get_capacity(),
                                  msgbuf.get_position())))
        {
          TBSYS_LOG(WARN, "fail to deserialize result code, ret = [%d]", ret);
        }
        /* sql result buffer will send no matter execute succ or not */
        else if (OB_SUCCESS !=
