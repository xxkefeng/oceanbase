...
namespace oceanbase
{
  namespace common
  {
    ...
    struct ColumnSchema
    {
      ...
      inline int64_t set_column_name(const ObString& name)
      {
        int64_t len = name.length() < OB_MAX_COLUMN_NAME_LENGTH - 1 ?
                      name.length() : OB_MAX_COLUMN_NAME_LENGTH - 1;
        memcpy(column_name_, name.ptr(), len);
        column_name_[len] = '\0';
        return len;
      }
    };
    ...
    public:
      ...
      inline void set_table_type(TableType table_type)
      {
      ...
      inline void set_data_table_id(uint64_t data_table_id)
      {
      ...
      inline void init_as_inner_table()
      {
        ...
        this->charset_number_ = OB_DEFAULT_CHARACTER_SET;
      ...
      inline int64_t set_table_name(const ObString& name)
      {
        int64_t len = name.length() < OB_MAX_TABLE_NAME_LENGTH - 1 ?
                      name.length() : OB_MAX_TABLE_NAME_LENGTH - 1;
        memcpy(table_name_, name.ptr(), len);
        table_name_[len] = '\0';
        return len;
      }

      inline int64_t set_compress_func_name(const ObString& name)
      {
        int64_t len = name.length() < OB_MAX_TABLE_NAME_LENGTH - 1 ?
                      name.length() : OB_MAX_TABLE_NAME_LENGTH - 1;
        memcpy(compress_func_name_, name.ptr(), len);
        compress_func_name_[len] = '\0';
        return len;
      }

      inline int64_t set_expire_condition(const ObString& expire_condition)
      {
        int64_t len = expire_condition.length() < OB_MAX_EXPIRE_CONDITION_LENGTH - 1 ?
                      expire_condition.length() : OB_MAX_EXPIRE_CONDITION_LENGTH - 1;
        memcpy(expire_condition_, expire_condition.ptr(), len);
        expire_condition_[len] = '\0';
        return len;
      }
    ...
    class ObSchemaService
    {
      public:
        ...
        virtual int modify_table_id(const TableSchema& table_schema, const int64_t new_table_id) = 0;
        virtual int modify_index_table_status(const ObString& index_table_name, const int32_t state) = 0;
