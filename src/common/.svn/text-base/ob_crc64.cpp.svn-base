  ...
  namespace common
  {
    ...
    */
                                       ...
                                       const char* buf, int64_t len)
    {
      ...
      return crc;
#else
      return ob_crc64_optimized(uCRC64, buf, len);
#endif /* defined(__GNUC__) && defined(__x86_64__) */
    }

    uint64_t ob_crc64_sse42(uint64_t uCRC64, const void *pv, int64_t cb)
    {
      return crc64_sse42(uCRC64, reinterpret_cast<const char*>(pv), cb);
    }

    uint64_t ob_crc64_sse42(const void *pv, int64_t cb)
    {
      return crc64_sse42(0, reinterpret_cast<const char*>(pv), cb);
    }
  }
}
