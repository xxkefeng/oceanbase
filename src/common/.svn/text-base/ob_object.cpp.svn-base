...
void ObObj::dump(const int32_t log_level /*=TBSYS_LOG_LEVEL_DEBUG*/) const
{
  ...
  switch (get_type())
  {
    ...
    case ObPreciseDateTimeType:
      ...
      break;
    case ObUnknownType:
      get_unknown(unknown_val);
      TBSYS_LOGGER.logMessage(TBSYS_LOG_NUM_LEVEL(log_level),
          "[%ld] type:ObUnknown, val:%p",pthread_self(),unknown_val);
      break;
    case ObCreateTimeType:
...
void ObObj::print_value(FILE* fd)
{
  switch (get_type())
  {
    ...
    case ObCreateTimeType:
      ...
      break;
    case ObUnknownType:
      fprintf(fd, "%p", value_.unknown_val);
      break;
    case ObExtendType:
...
int64_t ObObj::to_string(char* buffer, const int64_t length) const
{
  ...
  {
    databuff_printf(buffer, length, pos, "%s", "invalid");
  }

  {
    switch(meta_.type_)
    {
      ...
      case ObUnknownType:
        get_unknown(unknown_val);
        databuff_printf(buffer, length, pos, "%p", unknown_val);
        break;
      case ObCreateTimeType:
        get_createtime(int_val);
  ...
  if (OB_SUCCESS == ret)
  {
    switch (type)
    {
      ...
      case ObDecimalType:
        ...
        break;
      case ObUnknownType: // should not serialize
        TBSYS_LOG(ERROR, "ObUnknownType should not serialize");
