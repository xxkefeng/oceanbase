...
int ObRowUtil::convert(const ObString &compact_row, ObRow &row, ObRowkey *rowkey, ObObj *rowkey_buf)
{
  ...
  while(OB_SUCCESS == (ret = cell_reader.next_cell()))
  {
    ...
    {
      TBSYS_LOG(WARN, "failed to set cell, err=%d cell[%s]", ret, to_cstring(cell));
      break;
    }
  }
  ...
  if (cell_idx != row.get_column_num())
  {
    ...
    TBSYS_LOG(ERROR, "corrupted row data, row_desc[%s] col=%ld cell_num=%ld",
        to_cstring(*row_desc), row.get_column_num(), cell_idx);
