 ...
 *
 * Time-stamp: <2013-04-17 15:17:41 fufeng.syd>
 * Version: $Id$
...
int ObConfigManager::update_local()
{
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      if (NULL == dump_path_)
      {
        TBSYS_LOG(ERROR, "System config dump path not set, cann't dump");
      }
      else if (OB_SUCCESS != (ret = server_config_.reload()))
      {
    ...
    {
      TBSYS_LOG(ERROR,
                "read system config form inner talbe error, ret: [%d]", ret);
    }
  }
  return ret;
}
