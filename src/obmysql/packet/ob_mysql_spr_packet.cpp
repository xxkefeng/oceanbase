  ...
  namespace obmysql
  {
    ...
    int ObMySQLSPRPacket::serialize(char* buffer, int64_t len, int64_t& pos)
    {
      ...
      else
      {
        ...
        {
          TBSYS_LOG(ERROR, "serialize status failed, buffer=%p, len=%ld, status=%u, pos=%ld",
                    buffer, len, status_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int4(buffer, len, statement_id_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize warning_count failed, buffer=%p, len=%ld, statement_id_=%u, pos=%ld",
                    buffer, len, statement_id_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, column_num_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, column_num_=%u, pos=%ld",
                    buffer, len, column_num_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, param_num_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, param_num_=%u, pos=%ld",
                    buffer, len, param_num_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, reserved_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, reserved_=%u, pos=%ld",
                    buffer, len, reserved_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, warning_count_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, warning_count_=%u, pos=%ld",
                    buffer, len, warning_count_, pos);
        }
      }
      return ret;
    }
