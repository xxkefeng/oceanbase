  ...
  namespace obmysql
  {
    ...
    int ObMySQLEofPacket::serialize(char* buffer, int64_t len, int64_t& pos)
    {
      ...
      else
      {
        ...
        {
          TBSYS_LOG(ERROR, "serialize field_count failed, buffer=%p, len=%ld, field_count=%u, pos=%ld",
                    buffer, len, field_count_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, warning_count_, pos)))
        {
          TBSYS_LOG(ERROR, "serialize warning_count failed, buffer=%p, len=%ld, warning_count=%u, pos=%ld",
